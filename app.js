...
$('saveCustomerBtn').onclick=async()=>{ const id=$('customerEditId').value; const obj={code:$('cCode').value||code('KH'),name:$('cName').value,type:$('cType').value,phone:$('cPhone').value,address:$('cAddress').value,discount:+$('cDiscount').value||0,tax:$('cTax').value,note:$('cNote').value}; if(id){await updateDoc(doc(db,'customers',id),obj)}else{await addDoc(collection(db,'customers'),{...obj,createdAt:serverTimestamp()})} resetCustomer(); await loadAll(); };
function renderCustomers(){ $('customerTable').innerHTML=data.customers.map(x=>`<tr><td>${x.code||''}</td><td>${x.name||''}</td><td>${x.type||''}</td><td>${x.phone||''}</td><td>${x.address||''}</td><td>${x.discount||0}%</td><td><div class="actions"><button onclick="editCustomer('${x.id}')">Sửa</button><button class="danger" onclick="del('customers','${x.id}')">Xóa</button></div></td></tr>`).join(''); }
window.editCustomer=id=>{ const x=data.customers.find(i=>i.id===id); if(!x)return; $('customerEditId').value=id; $('cCode').value=x.code||''; $('cName').value=x.name||''; $('cType').value=x.type||'Khách lẻ'; $('cPhone').value=x.phone||''; $('cAddress').value=x.address||''; $('cDiscount').value=x.discount||0; $('cTax').value=x.tax||''; $('cNote').value=x.note||''; $('customerFormTitle').innerText='Sửa khách hàng'; window.scrollTo(0,0); };
window.resetCustomer=()=>{ ['customerEditId','cCode','cName','cPhone','cAddress','cTax','cNote'].forEach(i=>$(i).value=''); $('cDiscount').value=0; $('cType').value='Khách lẻ'; $('customerFormTitle').innerText='Thông tin khách hàng'; }; $('cancelCustomerBtn').onclick=resetCustomer;



$('saveProductBtn').onclick=async()=>{ const id=$('productEditId').value; const obj={code:$('pCode').value.trim(),name:$('pName').value,category:$('pCategory').value,unit:$('pUnit').value,cost:+$('pCost').value||0,price:+$('pPrice').value||0,stock:+$('pStock').value||0,note:$('pNote').value}; if(id){await updateDoc(doc(db,'products',id),obj)}else{await addDoc(collection(db,'products'),{...obj,createdAt:serverTimestamp()})} resetProduct(); await loadAll(); };
function renderProducts(){ $('productTable').innerHTML=data.products.map(x=>`<tr><td>${x.code}</td><td>${x.name}</td><td>${x.category||''}</td><td>${x.unit||'Bộ'}</td><td class="view-cost">${money(x.cost)}</td><td>${money(x.price)}</td><td>${x.stock||0}</td><td><div class="actions"><button onclick="editProduct('${x.id}')">Sửa</button><button class="danger" onclick="del('products','${x.id}')">Xóa</button></div></td></tr>`).join(''); }
window.editProduct=id=>{ const x=data.products.find(i=>i.id===id); if(!x)return; $('productEditId').value=id; $('pCode').value=x.code||''; $('pName').value=x.name||''; $('pCategory').value=x.category||''; $('pUnit').value=x.unit||'Bộ'; $('pCost').value=x.cost||0; $('pPrice').value=x.price||0; $('pStock').value=x.stock||0; $('pNote').value=x.note||''; $('productFormTitle').innerText='Sửa sản phẩm'; window.scrollTo(0,0); };
window.resetProduct=()=>{ ['productEditId','pCode','pName','pNote'].forEach(i=>$(i).value=''); $('pCategory').value='Khóa thông minh'; $('pUnit').value='Bộ'; $('pCost').value=0; $('pPrice').value=0; $('pStock').value=0; $('productFormTitle').innerText='Thông tin sản phẩm'; }; $('cancelProductBtn').onclick=resetProduct;



$('savePriceBtn').onclick=async()=>{ const p=productById($('plProductCode').value); if(!p)return alert('Chưa chọn sản phẩm'); const id=$('priceEditId').value; const obj={productId:p.id,code:p.code,customerType:$('plCustomerType').value,price:+$('plPrice').value||0,note:$('plNote').value}; if(id){await updateDoc(doc(db,'prices',id),obj)}else{await addDoc(collection(db,'prices'),{...obj,createdAt:serverTimestamp()})} resetPrice(); await loadAll(); };
function renderPrices(){ $('priceTable').innerHTML=data.prices.map(x=>`<tr><td>${x.code}</td><td>${x.customerType}</td><td>${money(x.price)}</td><td>${x.note||''}</td><td><div class="actions"><button onclick="editPrice('${x.id}')">Sửa</button><button class="danger" onclick="del('prices','${x.id}')">Xóa</button></div></td></tr>`).join(''); }
window.editPrice=id=>{ const x=data.prices.find(i=>i.id===id); if(!x)return; $('priceEditId').value=id; $('plProductCode').value=x.productId||''; $('plCustomerType').value=x.customerType||'Khách lẻ'; $('plPrice').value=x.price||0; $('plNote').value=x.note||''; };
window.resetPrice=()=>{ $('priceEditId').value=''; $('plPrice').value=''; $('plNote').value=''; };
...
$('saveStaffBtn').onclick=async()=>{ const id=$('staffEditId').value; const obj={name:$('eName').value,dept:$('eDept').value,phone:$('ePhone').value,email:$('eEmail').value}; if(id){await updateDoc(doc(db,'staff',id),obj)}else{await addDoc(collection(db,'staff'),{...obj,createdAt:serverTimestamp()})} resetStaff(); await loadAll(); };
function renderStaff(){ $('staffTable').innerHTML=data.staff.map(x=>`<tr><td>${x.name}</td><td>${x.dept}</td><td>${x.phone||''}</td><td>${x.email||''}</td><td><div class="actions"><button onclick="editStaff('${x.id}')">Sửa</button><button class="danger" onclick="del('staff','${x.id}')">Xóa</button></div></td></tr>`).join(''); }
window.editStaff=id=>{ const x=data.staff.find(i=>i.id===id); if(!x)return; $('staffEditId').value=id; $('eName').value=x.name||''; $('eDept').value=x.dept||'Sale'; $('ePhone').value=x.phone||''; $('eEmail').value=x.email||''; };
window.resetStaff=()=>{ $('staffEditId').value=''; $('eName').value=''; $('eDept').value='Sale'; $('ePhone').value=''; $('eEmail').value=''; };
