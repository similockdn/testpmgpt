...
function renderDebts(){
  const active=calcDebts();
  $('debtTable').innerHTML=active.map(d=>`<tr><td>${ensureCustomerCode(d.customer)}</td><td>${d.customer.name}</td><td>${money(d.total)}</td><td>${money(d.paid)}</td><td><b class="text-danger">${money(d.debt)}</b></td><td><button class="btn primary" onclick="receiptFor('${d.customer.id}')">Thu tiền</button></td></tr>`).join('')||'<tr><td colspan="6">Không còn công nợ phải thu</td></tr>';
  if($('settledDebtTable'))$('settledDebtTable').innerHTML=calcSettledDebts().map(d=>`<tr><td>${ensureCustomerCode(d.customer)}</td><td>${d.customer.name}</td><td>${money(d.total)}</td><td>${money(d.paid)}</td><td><span class="badge green">Đã thu đủ</span>${d.overPaid?` <span class="badge orange">Thu dư ${money(d.overPaid)}</span>`:''}</td></tr>`).join('')||'<tr><td colspan="5">Chưa có công nợ đã tất toán</td></tr>';
}
