...
window.editStaff=id=>{let e=data.staff.find(x=>x.id===id);$('eId').value=id;$('eName').value=e.name;$('eDept').value=e.dept;$('ePhone').value=e.phone||'';if($('eCommissionPercent'))$('eCommissionPercent').value=e.commissionPercent||5}

window.resetSaleForm=()=>{editingSale=null;$('saleCode').value=nextCode('BH',data.sales);$('saleDate').value=today();$('saleCustomerSearch').value='';$('salePaid').value=0;if($('saleCommissionPercent'))$('saleCommissionPercent').value=5;if($('saleTechCost'))$('saleTechCost').value=100000;$('saleNote').value='';$('saleItems').innerHTML='';addSaleItem();updateSaleTotals()}
...
function renderCommissions(){
  ...
  const by={};
  data.sales.forEach(s=>{let key=s.staffId||'none';let name=s.staffName||data.staff.find(x=>x.id===s.staffId)?.name||'Chưa chọn sale';by[key]=by[key]||{name,count:0,revenue:0,commission:0};by[key].count++;by[key].revenue+=+s.grand||0;by[key].commission+=+s.saleCommission||0});
  $('commissionByStaff').innerHTML=Object.values(by).sort((a,b)=>b.commission-a.commission).map(v=>`<tr><td>${v.name}</td><td>${v.count}</td><td>${money(v.revenue)}</td><td><b>${money(v.commission)}</b></td></tr>`).join('')||'<tr><td colspan="4">Chưa có dữ liệu hoa hồng</td></tr>';
  $('commissionByOrder').innerHTML=data.sales.slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(s=>`<tr><td>${s.date||''}</td><td>${s.code}</td><td>${s.customerName||''}</td><td>${s.staffName||data.staff.find(x=>x.id===s.staffId)?.name||''}</td><td>${money(s.grand)}</td><td>${s.commissionPercent||0}%</td><td><b>${money(s.saleCommission||0)}</b></td></tr>`).join('')||'<tr><td colspan="7">Chưa có đơn bán</td></tr>';
}
