...
let currentUser=null,currentPerm={role:'Admin',perms:[]},creatingAdmin=false;let editingSale=null,editingStock=null,editingWarranty=null,editingExpense=null,editingReceipt=null;
...
function warehouseOptions(selected=''){return WAREHOUSES.map(w=>`<option value="${w}" ${w===selected?'selected':''}>${w}</option>`).join('')}
function defaultWarehouse(){return WAREHOUSES[0]}
...
function renderSelectors(){fillSelect($('saleStaff'),data.staff.filter(x=>x.dept==='Sale'||x.dept==='Quản lý'),x=>x.name);fillSelect($('saleTech'),data.staff.filter(x=>x.dept==='Kỹ thuật'),x=>x.name);fillSelect($('commissionStaff'),data.staff,x=>`${x.name} - ${x.dept||''}`);fillSelect($('priceProduct'),data.products,x=>`${x.code} - ${x.name}`,x=>x.code);fillSelect($('receiptCustomer'),data.customers,x=>`${ensureCustomerCode(x)} - ${x.name} - ${x.phone||''}`);fillSelect($('wSale'),data.sales,x=>`${x.code} - ${x.customerName||''}`);if($('saleWarehouse'))$('saleWarehouse').innerHTML=warehouseOptions($('saleWarehouse').value||'Kho Văn Phòng');if($('stockWarehouse'))$('stockWarehouse').innerHTML=warehouseOptions($('stockWarehouse').value||defaultWarehouse());if($('stockToWarehouse'))$('stockToWarehouse').innerHTML=warehouseOptions($('stockToWarehouse').value||'Kho Văn Phòng');$('customerList').innerHTML=data.customers.map(c=>`<option value="${ensureCustomerCode(c)} | ${c.name} | ${c.phone||''}"></option>`).join('')}
...
window.saveSale=async()=>{let customer=findCustomerBySearch();if(!customer){await quickCreateCustomer();customer=findCustomerBySearch()}let items=saleItems();if(!items.length)return alert('Chưa có sản phẩm');
  ...
  if(exportStock){
    ...
    else{const vr=await addDoc(col('stockVouchers'),{...voucher,createdAt:serverTimestamp()});await updateDoc(doc(db,'sales',editingSale),{stockVoucherId:vr.id,stockVoucherCode:voucher.code})}
  }else if(oldSale?.stockVoucherId){
    await deleteDoc(doc(db,'stockVouchers',oldSale.stockVoucherId));
    await updateDoc(doc(db,'sales',editingSale),{stockVoucherId:'',stockVoucherCode:'',stockExported:false});
  }
...
function renderSales(){
  ...
  $('saleTable').innerHTML=rows.map(s=>{const pay=salePaymentInfo(s);const sv=stockVoucherForSale(s);return `<tr><td><b>${s.code}</b></td><td>${s.date||''}</td><td>${s.customerCode||''}</td><td>${s.customerName||''}<br><small>${s.customerPhone||''}</small></td><td><b>${money(s.grand)}</b></td><td>${money(pay.paidTotal)}</td><td><b>${money(pay.debtLeft)}</b></td><td class="view-cost">${money(s.saleCommission||0)}</td><td class="view-cost">${money(s.profit||0)}</td><td><span class="badge ${pay.debtLeft>0?(pay.paidTotal>0?'orange':'red'):'green'}">${pay.paymentStatus}</span></td><td>${sv?`<span class="badge green">${sv.code||'Đã xuất'}</span>`:'<span class="badge orange">Chưa xuất</span>'}</td><td><button class="btn ghost" onclick="viewSaleDetail('${s.id}')">Chi tiết</button> <button class="btn ghost" onclick="printSale('${s.id}')">In A5</button> ${sv?`<button class="btn ghost" onclick="printStock('${sv.id}')">Phiếu XK</button>`:''} ${has('editSales')?`<button class="btn ghost" onclick="editSale('${s.id}')">Sửa</button>`:''} ${has('deleteSales')?`<button class="btn danger" onclick="removeDoc('sales','${s.id}')">Xóa</button>`:''}</td></tr>`}).join('')||'<tr><td colspan="12">Chưa có phiếu bán</td></tr>';
}
...
function commissionFilteredSales(){
  let q=($('commissionSearch')?.value||'').toLowerCase().trim();
  let dept=$('commissionDept')?.value||'';
  let staffId=$('commissionStaff')?.value||'';
  let period=$('commissionPeriod')?.value||'all';
  let range=period==='custom'?{from:$('commissionFrom')?.value||'',to:$('commissionTo')?.value||''}:periodRange(period);
  if(period!=='custom'){
  ...
  return data.sales.filter(s=>{
    ...
    if(dept==='Sale' && !s.staffId) return false;
    if(dept==='Kỹ thuật' && !s.techId) return false;
    if(staffId){
      if(dept==='Sale' && s.staffId!==staffId) return false;
      if(dept==='Kỹ thuật' && s.techId!==staffId) return false;
      if(!dept && s.staffId!==staffId && s.techId!==staffId) return false;
    }
    if(q){
...
function renderCommissions(){
  ...
  $('commissionByStaff').innerHTML=Object.values(bySale)
    ...
    .join('')||'<tr><td colspan="4">Không có dữ liệu hoa hồng Sale theo bộ lọc</td></tr>';
...
function calcDebts(){return data.customers.map(c=>{let sales=data.sales.filter(s=>s.customerId===c.id);let total=sales.reduce((a,s)=>a+(+s.grand||0),0)+(+c.openingDebt||0);let paid=sales.reduce((a,s)=>a+(+s.paid||0),0)+data.receipts.filter(r=>r.customerId===c.id).reduce((a,r)=>a+(+r.amount||0),0);let rawDebt=total-paid;return{customer:c,total,paid,debt:Math.max(0,rawDebt),overPaid:Math.max(0,-rawDebt)}}).filter(x=>x.total||x.paid||x.debt||x.overPaid)}
function renderDebts(){$('debtTable').innerHTML=calcDebts().map(d=>`<tr><td>${ensureCustomerCode(d.customer)}</td><td>${d.customer.name}</td><td>${money(d.total)}</td><td>${money(d.paid)}</td><td><b>${money(d.debt)}</b>${d.overPaid?`<br><span class="badge orange">Thu dư ${money(d.overPaid)}</span>`:''}</td><td><button class="btn ghost" onclick="receiptFor('${d.customer.id}')">Thu tiền</button></td></tr>`).join('')}
window.resetReceiptForm=()=>{editingReceipt=null;$('receiptCustomer').value='';$('receiptAmount').value='';$('receiptDate').value=today();$('receiptNote').value=''}
window.receiptFor=id=>{resetReceiptForm();$('receiptCustomer').value=id;$('receiptDate').value=today();showPage('debts')};window.openReceiptForm=()=>{resetReceiptForm();showPage('debts')}
window.saveReceipt=async()=>{let cid=$('receiptCustomer').value,amount=+$('receiptAmount').value||0;if(!cid||!amount)return alert('Chọn khách và nhập số tiền');let d=calcDebts().find(x=>x.customer.id===cid);if(d&&amount>d.debt&&!confirm(`Số tiền thu ${money(amount)} lớn hơn công nợ còn lại ${money(d.debt)}. Vẫn lưu phiếu thu?`))return;let c=data.customers.find(x=>x.id===cid)||{};let o={customerId:cid,customerCode:ensureCustomerCode(c),customerName:c.name||'',amount,date:$('receiptDate').value||today(),note:$('receiptNote').value||'',updatedAt:serverTimestamp()};if(editingReceipt){await updateDoc(doc(db,'receipts',editingReceipt),o);await logAction('Sửa phiếu thu',o.customerName+' '+o.amount)}else{await addDoc(col('receipts'),{code:nextCode('PT',data.receipts),...o,createdAt:serverTimestamp()});await logAction('Thêm phiếu thu',o.customerName+' '+o.amount)}resetReceiptForm();await loadAll();await updatePaymentStatusesForCustomer(cid);await loadAll()}
...
window.resetStockForm=()=>{editingStock=null;$('stockCode').value=nextCode(prefixByStockType($('stockType').value||'IN'),data.stockVouchers);$('stockDate').value=today();$('stockType').value=$('stockType').value||'IN';$('stockWarehouse').innerHTML=warehouseOptions(defaultWarehouse());$('stockWarehouse').value=defaultWarehouse();if($('stockToWarehouse')){$('stockToWarehouse').innerHTML=warehouseOptions('Kho Văn Phòng');$('stockToWarehouse').value='Kho Văn Phòng';}$('stockNote').value='';$('stockItems').innerHTML='';addStockItem();updateStockHeader()};
  ...
  $('stockBookTable').innerHTML=data.products.map(p=>{
    ...
    data.stockVouchers.forEach(v=>(v.items||[]).forEach(it=>{
      ...
      if(v.type==='TRANSFER'){
        ...
        totalTransfer+=q; byWh[from]=(byWh[from]||0)-q; byWh[to]=(byWh[to]||0)+q;
      }else{
    ...
    const total=(byWh['Kho Chính']||0)+(byWh['Kho Văn Phòng']||0);
    return`<tr><td>${p.code}</td><td>${p.name}</td><td>${totalIn}</td><td>${totalOut}</td><td>${totalTransfer}</td><td>${totalAdj}</td><td><b>${byWh['Kho Chính']||0}</b></td><td><b>${byWh['Kho Văn Phòng']||0}</b></td><td><b>${total}</b></td></tr>`
  }).join('')
}
...
window.clearAllData=async()=>{
  ...
  try{
    ...
    const collectionsToClear=['customers','products','prices','staff','sales','stockVouchers','receipts','warranties','expenses','logs'];
    let result=[];
