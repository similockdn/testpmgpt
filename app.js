...
function renderSales(){let q=($('saleSearch')?.value||'').toLowerCase();$('saleTable').innerHTML=data.sales.filter(s=>(s.code+(s.customerCode||'')+s.customerName).toLowerCase().includes(q)).sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(s=>`<tr><td>${s.code}</td><td>${s.date||''}</td><td>${s.customerCode||''}</td><td>${s.customerName||''}</td><td>${money(s.grand)}</td><td>${money(s.paid)}</td><td>${money(s.debt)}</td><td class="view-cost">${money(s.saleCommission||0)}</td><td class="view-cost">${money(s.profit||0)}</td><td><span class="badge ${s.debt>0?'orange':'green'}">${s.status}</span></td><td>${s.stockExported?'<span class="badge green">Đã xuất</span>':'<span class="badge orange">Chưa xuất</span>'}</td><td><button class="btn ghost" onclick="printSale('${s.id}')">In A5</button> ${has('editSales')?`<button class="btn ghost" onclick="editSale('${s.id}')">Sửa</button>`:''} ${has('deleteSales')?`<button class="btn danger" onclick="removeDoc('sales','${s.id}')">Xóa</button>`:''}</td></tr>`).join('')}
...
window.saveReceipt=async()=>{let cid=$('receiptCustomer').value,amount=+$('receiptAmount').value||0;if(!cid||!amount)return alert('Chọn khách và nhập số tiền');let c=data.customers.find(x=>x.id===cid)||{};let o={customerId:cid,customerCode:ensureCustomerCode(c),customerName:c.name||'',amount,date:$('receiptDate').value||today(),note:$('receiptNote').value||'',updatedAt:serverTimestamp()};if(editingReceipt){await updateDoc(doc(db,'receipts',editingReceipt),o);await logAction('Sửa phiếu thu',o.customerName+' '+o.amount)}else{await addDoc(col('receipts'),{code:nextCode('PT',data.receipts),...o,createdAt:serverTimestamp()});await logAction('Thêm phiếu thu',o.customerName+' '+o.amount)}resetReceiptForm();await loadAll()}
