...
async function loadAll(){for(const n of ['customers','products','staff','prices','sales','stockVouchers','receipts','warranties','users','logs']) await loadCol(n); renderAll();}
...
function renderReports(){
  let rev=data.sales.reduce((a,s)=>a+(+s.grand||0),0),profit=data.sales.reduce((a,s)=>a+(+s.profit||0),0),debt=calcDebts().reduce((a,d)=>a+d.debt,0),stock=data.products.reduce((a,p)=>a+stockOf(p.code),0);
  let logs=(data.logs||[]).slice().sort((a,b)=>String(b.at?.seconds||b.at||'').localeCompare(String(a.at?.seconds||a.at||''))).slice(0,20);
  $('reportBox').innerHTML=`<div class="report-card">Tổng doanh thu<b>${money(rev)}</b></div><div class="report-card view-cost">Tổng lợi nhuận<b>${money(profit)}</b></div><div class="report-card">Công nợ<b>${money(debt)}</b></div><div class="report-card">Tổng tồn kho<b>${stock}</b></div>`+
...
window.exportBackup=()=>{
  const pack={exportedAt:new Date().toISOString(),customers:data.customers,products:data.products,prices:data.prices,staff:data.staff,sales:data.sales,stockVouchers:data.stockVouchers,receipts:data.receipts,warranties:data.warranties,users:data.users};
