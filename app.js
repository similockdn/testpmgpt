...
function stockOf(code,excludeVoucherId=''){
  let qty=0;
  // Tồn kho chỉ tính theo chứng từ kho. Đơn bán chỉ trừ kho khi có tick "Kiêm xuất kho" và tạo phiếu xuất kho OUT riêng.
...
window.saveSale=async()=>{let customer=findCustomerBySearch();if(!customer){await quickCreateCustomer();customer=findCustomerBySearch()}let items=saleItems();if(!items.length)return alert('Chưa có sản phẩm');
  ...
  if(exportStock){
    const voucher={code:oldSale?.stockVoucherCode||nextCode('XK',data.stockVouchers),date:o.date,type:'OUT',warehouse:'Kho SIMILOCK',saleId:editingSale,saleCode:o.code,customerName:o.customerName,note:`Xuất kho theo đơn bán ${o.code}`,items:voucherItemsFromSaleItems(items),value:cost,updatedAt:serverTimestamp()};
  ...
  editingSale=null;await loadAll();resetSaleForm()}
...
function renderDebts(){$('debtTable').innerHTML=calcDebts().map(d=>`<tr><td>${ensureCustomerCode(d.customer)}</td><td>${d.customer.name}</td><td>${money(d.total)}</td><td>${money(d.paid)}</td><td><b>${money(d.debt)}</b></td><td><button class="btn ghost" onclick="receiptFor('${d.customer.id}')">Thu tiền</button></td></tr>`).join('')}
...
function renderReceipts(){$('receiptTable').innerHTML=data.receipts.slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(r=>`<tr><td>${r.code||''}</td><td>${r.date||''}</td><td>${r.customerCode||''}</td><td>${r.customerName||''}</td><td>${money(r.amount)}</td><td>${r.note||''}</td><td><button class="btn ghost" onclick="editReceipt('${r.id}')">Sửa</button> <button class="btn danger" onclick="removeDoc('receipts','${r.id}')">Xóa</button></td></tr>`).join('')||'<tr><td colspan="7">Chưa có phiếu thu</td></tr>'}
...
function updateStockHeader(){
  ...
  if(ths[2]) ths[2].textContent=isCheck?'Tồn thực tế':(type==='OUT'?'Số lượng xuất':(type==='ADJUST'?'SL điều chỉnh (+/-)':'Số lượng nhập'));
  if(ths[4]) ths[4].textContent=isCheck?'Ghi chú kiểm kê':(type==='OUT'?'Lý do xuất':(type==='ADJUST'?'Lý do điều chỉnh':'Ghi chú'));
}
