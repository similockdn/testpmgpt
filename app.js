...
const money=n=>(Number(n)||0).toLocaleString('vi-VN')+'đ';const today=()=>isoDateFromLocal(new Date());const uid=()=>Math.random().toString(36).slice(2,9);const normEmail=v=>String(v||'').trim().toLowerCase();
...
const DEFAULT_WAREHOUSE_RECORDS=[
  {code:'KCH',name:'Kho Chính',address:'Đà Nẵng',protected:true},
  {code:'KVP',name:'Kho Văn Phòng',address:'Đà Nẵng',protected:true},
  {code:'HCM',name:'Kho HCM',address:'TP.HCM',protected:false}
];
const PROTECTED_WAREHOUSE_NAMES=['Kho Chính','Kho Văn Phòng'];
...
function warehouseOverrideDefaultName(row){return String(row?.defaultName||'').trim()}
function warehouseMasterRows(includeInactive=true){
  const docs=(data.warehouses||[]);
  const hiddenDefaults=new Set(docs.map(warehouseOverrideDefaultName).filter(Boolean).map(searchKey));
  const defaults=DEFAULT_WAREHOUSE_RECORDS.filter(row=>!hiddenDefaults.has(searchKey(row.name))).map(row=>({...row,id:'',active:'active',isDefault:true}));
  const custom=docs.filter(row=>row.active!=='deleted'&&(includeInactive||row.active!=='inactive')).map(row=>({...row,isDefault:!!warehouseOverrideDefaultName(row)}));
  const seen=new Set();
  return [...defaults,...custom].filter(row=>{
    const key=searchKey(row.name);if(!key||seen.has(key))return false;seen.add(key);return true;
  }).sort((a,b)=>String(a.name).localeCompare(String(b.name),'vi'));
}
function warehouseNames(includeInactive=false){return warehouseMasterRows(includeInactive).filter(row=>includeInactive||row.active!=='inactive').map(row=>row.name)}
function refreshWarehouseCache(){
  const names=warehouseNames(false);
  WAREHOUSES.splice(0,WAREHOUSES.length,...(names.length?names:DEFAULT_WAREHOUSE_RECORDS.map(x=>x.name)));
}
function warehouseMasterByName(name){const key=searchKey(name);return warehouseMasterRows(true).find(row=>searchKey(row.name)===key)||null}
function activeSuppliers(){return (data.suppliers||[]).filter(row=>row.active!=='inactive'&&row.active!=='deleted').sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'vi'))}
...
function userWarehouses(){
  ...
  if(currentPerm.role==='Kho Văn Phòng')return ['Kho Văn Phòng'];
...
function canAccessVoucher(v){
  ...
  // là đối tượng bên ngoài hệ thống, không phải một kho nội bộ.
  if(v.type==='TRANSFER')return canAccessWarehouse(v.fromWarehouse||v.warehouse||'Kho Chính')||canAccessWarehouse(v.toWarehouse||'');
  if(v.type==='OUT')return canAccessWarehouse(v.fromWarehouse||v.warehouse||'Kho Chính');
  return canAccessWarehouse(voucherWarehouse(v));
}
function warehouseOptions(selected='',allowed=userWarehouses()){return allowed.map(w=>`<option value="${htmlesc(w)}" ${w===selected?'selected':''}>${htmlesc(w)}</option>`).join('')}
...
function transferStatusOf(v){
  if(v?.type!=='TRANSFER')return '';
  if(isVoucherCanceled(v)||v.transferStatus==='CANCELED')return 'CANCELED';
  // Phiếu cũ chưa có trạng thái đã được cộng tồn ở các phiên bản trước.
  return v.transferStatus||'RECEIVED';
}
function transferIsReceived(v){return ['RECEIVED','RECEIVED_WITH_DIFFERENCE'].includes(transferStatusOf(v))}
function transferReceivedQty(v,it,index){
  const sent=Math.abs(+it?.qty||0);
  if(v?.type!=='TRANSFER')return 0;
  if(!v.transferStatus)return sent;
  if(!transferIsReceived(v))return 0;
  const received=(v.receivedItems||[]).find(row=>+row.lineIndex===index||(!Number.isFinite(+row.lineIndex)&&row.code===it.code));
  // Phiếu mới đã có trạng thái nhưng thiếu dòng xác nhận thì fail-closed: không
  // cộng tồn. Chỉ phiếu cũ hoàn toàn chưa có transferStatus mới được coi là đã nhận đủ.
  if(!received)return 0;
  return Math.max(0,Math.min(sent,+received.receivedQty||0));
}
function transferShortageQty(v,it,index){return Math.max(0,Math.abs(+it?.qty||0)-transferReceivedQty(v,it,index))}
function transferStatusLabel(v){return ({IN_TRANSIT:'Đang vận chuyển',RECEIVED:'Đã nhận đủ',RECEIVED_WITH_DIFFERENCE:'Đã nhận có lệch',CANCELED:'Đã hủy'})[transferStatusOf(v)]||''}
function transferStatusBadge(v){const s=transferStatusOf(v);const cls=s==='IN_TRANSIT'?'orange':(s==='RECEIVED'?'green':(s==='RECEIVED_WITH_DIFFERENCE'?'red':'gray'));return `<span class="badge ${cls}">${transferStatusLabel(v)}</span>`}
...
window.applyRolePreset=()=>{
  ...
  document.querySelectorAll('#warehouseAccessBox input').forEach(i=>{i.checked=role==='Admin'||role==='Kho'||(role==='Kho Chính'&&i.value==='Kho Chính')||(role==='Kho Văn Phòng'&&i.value==='Kho Văn Phòng')});
  lockWarehouseScopeChooser();
};
function lockWarehouseScopeChooser(){
  const role=$('uRole')?.value||'';
  document.querySelectorAll('#warehouseAccessBox input').forEach(i=>{
    const fixed=role==='Admin'||role==='Kho Chính'||role==='Kho Văn Phòng';
    i.disabled=fixed;
    if(role==='Admin')i.checked=true;
    if(role==='Kho Chính')i.checked=i.value==='Kho Chính';
    if(role==='Kho Văn Phòng')i.checked=i.value==='Kho Văn Phòng';
  });
  const note=$('warehouseAccessNote');
  if(note)note.textContent=role==='Kho Chính'
    ?'Kho Chính: chỉ xem nhập–xuất–tồn Kho Chính; vẫn được chọn Kho Văn Phòng làm kho nhận khi chuyển kho.'
    :role==='Kho Văn Phòng'
      ?'Kho Văn Phòng: chỉ xem nhập–xuất–tồn Kho Văn Phòng.'
      :'Chọn các kho tài khoản được xem và thao tác.';
}
...
function normalizePermission(p={}){
  ...
  if(role==='Kho Chính')wh=['Kho Chính'];
...
function warehouseScopedUser(){return currentPerm.role==='Kho Chính'||currentPerm.role==='Kho Văn Phòng'||(currentPerm.role==='Kho'&&userWarehouses().length<WAREHOUSES.length)}
async function loadWarehouseScopedStockVouchers(){
  const warehouses=userWarehouses();
  if(!warehouses.length)return [];
  // Tải chứng từ phát sinh tại kho được giao và phiếu chuyển đến kho đó.
  // Gộp theo ID để phiếu chuyển không bị lặp giữa các truy vấn.
  const snapshots=await Promise.all(warehouses.flatMap(warehouse=>[
    getDocs(query(col('stockVouchers'),where('warehouse','==',warehouse))),
    getDocs(query(col('stockVouchers'),where('fromWarehouse','==',warehouse))),
    getDocs(query(col('stockVouchers'),where('toWarehouse','==',warehouse)))
  ]));
  const byId=new Map();
  snapshots.forEach(s=>s.docs.forEach(d=>byId.set(d.id,{id:d.id,...d.data()})));
  return [...byId.values()];
}
async function loadCol(n){try{if(n==='stockVouchers'&&warehouseScopedUser())data[n]=await loadWarehouseScopedStockVouchers();else{const s=await getDocs(col(n));data[n]=s.docs.map(d=>({id:d.id,...d.data()}));}}catch(e){console.warn('Không tải được collection '+n,e.message);data[n]=[];}}
async function loadAll(){
  await loadCol('warehouses');
  ...
  data.stockVouchers.forEach(v=>{
    ...
    (v.items||[]).forEach(it=>{
      ...
      if(v.type==='TRANSFER'){
        ...
        const to=v.toWarehouse||'';
        const index=(v.items||[]).indexOf(it);
        if(warehouse===from) qty-=q;
        if(warehouse===to&&transferIsReceived(v))qty+=transferReceivedQty(v,it,index);
      }else{
...
function visibleStockOf(code,excludeVoucherId=''){
  return userWarehouses().reduce((sum,warehouse)=>sum+stockOf(code,excludeVoucherId,warehouse),0);
}
...
function stockLedgerRows(){
  ...
  activeStockVouchers().forEach(v=>{
    (v.items||[]).forEach((it,index)=>{
      const q=+it.qty||0;
      if(v.type==='TRANSFER'){
        const receivedQty=transferReceivedQty(v,it,index);
        rows.push({date:v.date,code:v.code,type:'Chuyển đi',warehouse:v.fromWarehouse||v.warehouse||'',product:it.code,name:it.name,qty:-Math.abs(q),note:[v.note||it.note||'',transferStatusLabel(v)].filter(Boolean).join(' · ')});
        if(transferIsReceived(v)&&receivedQty>0)rows.push({date:v.receivedDate||v.date,code:v.code,type:'Chuyển đến',warehouse:v.toWarehouse||'',product:it.code,name:it.name,qty:receivedQty,note:[v.receiveNote||'',transferStatusLabel(v)].filter(Boolean).join(' · ')});
      }else{
...
function stockBookRows(from='',to=''){
  ...
  const emptyWarehouseStock=()=>Object.fromEntries(WAREHOUSES.map(name=>[name,0]));
  (data.products||[]).forEach(p=>{
    productMap.set(p.code,{
      ...
      warehouseStock:emptyWarehouseStock(),
      warehouseInTransit:emptyWarehouseStock(),
      warehouseDifference:emptyWarehouseStock(),
      stock:0,
      totalIn:0,
      totalOut:0,
      totalTransfer:0,
  ...
  function ensure(code,name,cost=0){
    ...
    if(!productMap.has(code)){
      productMap.set(code,{code,name:name||'',cost:+cost||0,minStock:3,active:'active',warehouseStock:emptyWarehouseStock(),warehouseInTransit:emptyWarehouseStock(),warehouseDifference:emptyWarehouseStock(),stock:0,totalIn:0,totalOut:0,totalTransfer:0,inTransit:0,pendingDifference:0,totalAdj:0,periodMovement:false,value:0});
    }
  ...
  function addWarehouseStock(row,warehouse,qty){
    if(!warehouse)return;
    if(!(warehouse in row.warehouseStock))row.warehouseStock[warehouse]=0;
    row.warehouseStock[warehouse]+=qty;
  }
  activeStockVouchers().forEach(v=>{
    ...
    (v.items||[]).forEach((it,index)=>{
      ...
      if(v.type==='TRANSFER'){
        addWarehouseStock(r,fromWh,-q);
        const receivedQty=transferReceivedQty(v,it,index);
        if(transferIsReceived(v))addWarehouseStock(r,toWh,receivedQty);
        if(transferStatusOf(v)==='IN_TRANSIT'&&(canAccessWarehouse(fromWh)||canAccessWarehouse(toWh))){r.inTransit+=q;if(r.warehouseInTransit[fromWh]!==undefined)r.warehouseInTransit[fromWh]+=q;if(r.warehouseInTransit[toWh]!==undefined)r.warehouseInTransit[toWh]+=q;}
        if(transferStatusOf(v)==='RECEIVED_WITH_DIFFERENCE'&&(canAccessWarehouse(fromWh)||canAccessWarehouse(toWh))){const shortage=Math.max(0,q-receivedQty);r.pendingDifference+=shortage;if(r.warehouseDifference[fromWh]!==undefined)r.warehouseDifference[fromWh]+=shortage;if(r.warehouseDifference[toWh]!==undefined)r.warehouseDifference[toWh]+=shortage;}
        if(inPeriod&&(canAccessWarehouse(fromWh)||canAccessWarehouse(toWh))){r.totalTransfer+=q;r.periodMovement=true;}
      }else if(v.type==='OUT'){
        addWarehouseStock(r,wh,-q);
        if(inPeriod&&canAccessWarehouse(wh)){r.totalOut+=q;r.periodMovement=true;}
      }else if(v.type==='IN'||v.type==='RETURN'){
        addWarehouseStock(r,wh,q);
        if(inPeriod&&canAccessWarehouse(wh)){r.totalIn+=q;r.periodMovement=true;}
      }else{
        const signed=+it.qty||0;
        addWarehouseStock(r,wh,signed);
        if(inPeriod&&canAccessWarehouse(wh)){r.totalAdj+=signed;r.periodMovement=true;}
      }
    });
  });
  return [...productMap.values()].map(r=>{
    const visibleWarehouseStock={};
    Object.entries(r.warehouseStock).forEach(([name,qty])=>{visibleWarehouseStock[name]=canAccessWarehouse(name)?qty:0});
    const stock=Object.values(visibleWarehouseStock).reduce((sum,qty)=>sum+(+qty||0),0);
    return {...r,warehouseStock:visibleWarehouseStock,khoChinh:visibleWarehouseStock['Kho Chính']||0,khoVanPhong:visibleWarehouseStock['Kho Văn Phòng']||0,khoHCM:visibleWarehouseStock['Kho HCM']||0,stock,value:stock*(+r.cost||0)};
  }).sort((a,b)=>String(a.code).localeCompare(String(b.code),'vi',{numeric:true}));
}

function printHeader(title){return `<p style="text-align:center;line-height:1.45;margin:0 0 6px"><b>SIMILOCK ĐÀ NẴNG</b><br>223 Trường Chinh, P. An Khê, TP. Đà Nẵng<br>403 Nguyễn Thái Bình, P. Bảy Hiền, TP.HCM<br>Hotline: 0902.950.816</p><h2 style="text-align:center;margin:4px 0 8px;font-size:18px">${title}</h2><hr>`}
...
function customerInfo(c={}){
  ...
  return {name,code,phone,address:c.address||c.customerAddress||'',type:c.type||c.customerType||c.customerGroup||'Khách lẻ',source:c.source||c.customerSource||''};
}
...
function saleCustomerInfo(s={}){
  ...
  const source=snap.source||s.customerSource||c.source||'';
  ...
  return {name,code,phone,address,type,source};
}
function customerSnapshotFromCustomer(c={}, overrideType=''){
  const i=customerInfo(c);
  const type=overrideType||i.type||'Khách lẻ';
  return {id:c.id||'', code:i.code||'', customerCode:i.code||'', name:i.name||'', phone:i.phone||'', address:i.address||'', type, source:i.source||''};
}
function customerSnapshotPayload(snap={}){
  return {
    customerId:snap.id||'', customerCode:snap.code||snap.customerCode||'', customerName:snap.name||'',
    customerPhone:snap.phone||'', customerAddress:snap.address||'', customerType:snap.type||'Khách lẻ', customerGroup:snap.type||'Khách lẻ', customerSource:snap.source||'',
    customerSnapshot:{id:snap.id||'', code:snap.code||snap.customerCode||'', name:snap.name||'', phone:snap.phone||'', address:snap.address||'', type:snap.type||'Khách lẻ', source:snap.source||''}
  };
}
...
function calcCommissionBase(totals){
  // Doanh số tính hoa hồng = Tổng thanh toán - VAT.
  // Phụ thu nằm trong Tổng thanh toán và KHÔNG bị trừ ra, vì vậy luôn được tính hoa hồng.
  return Math.max(0,(+totals?.grand||0)-(+totals?.vat||0));
}
...
function saleCommissionBaseValue(s){
  if(isSaleCanceled(s))return 0;
  // Luôn tính lại từ tổng tiền và VAT hiện tại để dữ liệu cũ/stale commissionBase
  // không làm sai hoa hồng sau khi sửa đơn hoặc trả hàng.
  if(Number.isFinite(+s?.grand))return calcCommissionBase(s);
  return Math.max(0,+s?.commissionBase||0);
}
function saleExpectedCommissionValue(s){
  if(isSaleCanceled(s))return 0;
  const hasPercent=s?.commissionPercent!==undefined&&s?.commissionPercent!==null&&String(s.commissionPercent)!=='';
  if(hasPercent)return Math.round(saleCommissionBaseValue(s)*(+s.commissionPercent||0)/100);
  // Dữ liệu mới lưu riêng số dự kiến và số đã ghi nhận; dữ liệu rất cũ có thể chỉ có saleCommission.
  return Math.max(0,+s?.expectedSaleCommission||+s?.saleCommission||0);
}
function saleCommissionValue(s){return saleFullyPaidForCommission(s)?saleExpectedCommissionValue(s):0}
function saleCostValue(s){
  if(isSaleCanceled(s))return 0;
  const storedCost=Number(s?.cost);
  const hasStoredCost=s?.cost!==undefined&&s?.cost!==null&&String(s.cost)!==''&&Number.isFinite(storedCost)&&storedCost>=0;
  return hasStoredCost?storedCost:((s?.items||[]).reduce((a,it)=>a+costFor(it.code,s.date||today())*(+it.qty||0),0));
}
function saleOrderNonCommissionExpenseValue(s){
  if(isSaleCanceled(s))return 0;
  // Chi phí của đơn không gồm hoa hồng Sale để các chỉ tiêu sau khấu trừ
  // luôn có thể đối chiếu theo công thức: Doanh thu - Chi phí - Hoa hồng.
  return saleCostValue(s)+(+s?.techCost||0)+(+s?.techFuel||0);
}
function revenueDeductionMetrics(sales=[],expenses=[],salaries=[]){
  const revenue=sales.reduce((a,s)=>a+(+s?.grand||0),0);
  const orderExpense=sales.reduce((a,s)=>a+saleOrderNonCommissionExpenseValue(s),0);
  const operatingExpense=expenses.reduce((a,e)=>a+(+e?.amount||0),0);
  const salaryExpense=salaries.reduce((a,e)=>a+(+e?.total||+e?.amount||0),0);
  const expense=orderExpense+operatingExpense+salaryExpense;
  const commission=sales.reduce((a,s)=>a+saleCommissionValue(s),0);
  return {
    revenue,orderExpense,operatingExpense,salaryExpense,expense,commission,
    afterExpense:revenue-expense,
    afterCommission:revenue-commission,
    afterExpenseCommission:revenue-expense-commission
  };
}
function saleProfitValue(s){
  if(isSaleCanceled(s))return 0;
  const cost=saleCostValue(s);
  // Lợi nhuận đơn hàng dùng chi phí dự kiến theo đơn, không phụ thuộc thời điểm khách trả tiền.
  return saleCommissionBaseValue(s)-cost-saleExpectedCommissionValue(s)-(+s?.techCost||0)-(+s?.techFuel||0);
}
...
function saleItemCommissionBaseShare(s={},item={}){
  const items=s.items||[];
  const lineTotal=items.reduce((a,it)=>a+lineNet(it),0);
  if(lineTotal<=0)return 0;
  const goodsBase=Math.max(0,saleCommissionBaseValue(s)-(+s.surcharge||0));
  return goodsBase*lineNet(item)/lineTotal;
}
...
function suggestedTechCost(){return techFeeDefault($('saleTech')?.value)}
...
function btnTitle(id){return ({dashboard:'Dashboard điều hành',sales:'Bán hàng',commissions:'Thu nhập nhân viên',expenses:'Phiếu chi / Chi phí',cashbook:'Sổ quỹ',salaries:'Lương nhân viên',debts:'Công nợ',inventory:'Kho hàng',stockbook:'Sổ kho',warehouseMaster:'Danh mục kho & nhà cung cấp',warranty:'Bảo hành',customers:'Khách hàng',products:'Sản phẩm',categories:'Danh mục chung',prices:'Bảng giá',categories:'Danh mục chung',staff:'Nhân viên',reports:'Báo cáo',categories:'Danh mục chung',permissions:'Phân quyền',system:'Hệ thống',audit:'Nhật ký thao tác'}[id]||id)}
...
function renderAll(){
  ...
  const steps=[
    ...
    ['renderWarehouseMaster',()=>renderWarehouseMaster()],
...
function renderWarehouseFilterOptions(id){
  const select=$(id);if(!select)return;
  const current=select.value||'ALL';
  const allowed=userWarehouses();
  select.innerHTML='<option value="ALL">Tất cả kho được phân quyền</option>'+warehouseOptions(current,allowed);
  select.value=current==='ALL'||allowed.includes(current)?current:'ALL';
}
function renderSelectors(){fillSelect($('saleStaff'),data.staff.filter(x=>staffHasFunction(x,'Sale')||staffHasFunction(x,'Quản lý')),x=>`${x.name}${staffHasFunction(x,'Kỹ thuật')?' (Sale + Kỹ thuật)':''}`);fillSelect($('saleTech'),data.staff.filter(x=>staffHasFunction(x,'Kỹ thuật')),x=>`${x.name}${staffHasFunction(x,'Sale')?' (Kỹ thuật + Sale)':''}`);refreshCommissionStaffOptions();ensureProductDatalist();fillReceiptCustomerOptions();renderProductCategoryOptions();renderWarrantyReasonOptions();renderExpenseCategoryOptions();renderPaymentMethodOptions();renderSystemCategories();renderWarrantyStaffSelectors();if($('saleWarehouse'))$('saleWarehouse').innerHTML=warehouseOptions($('saleWarehouse').value||defaultWarehouse());if($('stockWarehouse'))$('stockWarehouse').innerHTML=warehouseOptions($('stockWarehouse').value||defaultWarehouse());if($('stockSupplier')){$('stockSupplier').innerHTML=supplierOptions($('stockSupplier').value||'');}syncTransferDestination();renderWarehouseFilterOptions('stockBookWarehouse');renderWarehouseFilterOptions('reportStockWarehouse');if($('customerList')) $('customerList').innerHTML=data.customers.map(c=>`<option value="${customerSearchValue(c)}"></option>`).join('')}
...
function dashboardWeekSalesRows(referenceDate=today()){
  ...
  activeSales().forEach(s=>{
    const row=byDate[saleRevenueRecognitionDate(s)];
    if(!row)return;
...
function sumStockValue(){return data.products.reduce((a,p)=>a+(visibleStockOf(p.code)*(costFor(p.code,today())||+p.cost||0)),0)}
function renderDashboard(){
  ...
  //
  // Doanh thu = tổng giá trị các Phiếu bán đã thu đủ 100%,
  // theo ngày khoản thu làm số tiền lũy kế của phiếu đạt đủ 100%.
  ...
  const collected=orderSalesInRange.reduce((a,s)=>a+saleCollectedInRange(s,range.from,range.to),0);
  const cashIn=cashRowsInRange.reduce((a,r)=>a+(+r.income||0),0);
  const cashOut=cashRowsInRange.reduce((a,r)=>a+(+r.expense||0),0);
  const orderProfit=salesInRange.reduce((a,s)=>a+saleProfitValue(s),0);
  ...
  const revenueMetrics=revenueDeductionMetrics(salesInRange,monthlyExpenses,monthlySalaries);
  ...
  const low=data.products.filter(p=>visibleStockOf(p.code)<=(+p.minStock||3)).sort((a,b)=>visibleStockOf(a.code)-visibleStockOf(b.code));
  ...
  activeSales().forEach(s=>{const row=last12.find(x=>x.key===monthKey(saleRevenueRecognitionDate(s)));if(row){row.value+=+s.grand||0;row.profit+=saleProfitValue(s);}});
  const productMap={};salesInRange.forEach(s=>(s.items||[]).forEach(it=>{const p=data.products.find(x=>x.code===it.code)||{};productMap[it.code]=productMap[it.code]||{code:it.code,name:p.name||'',qty:0,revenue:0};productMap[it.code].qty+=+it.qty||0;productMap[it.code].revenue+=saleItemCommissionBaseShare(s,it);}));
  const productRows=Object.values(productMap).sort((a,b)=>b.qty-a.qty);
  const stockRows=data.products.map(p=>({label:p.code,value:visibleStockOf(p.code),stockValue:visibleStockOf(p.code)*(costFor(p.code,today())||+p.cost||0)})).filter(x=>x.value>0).sort((a,b)=>b.value-a.value);
  ...
  const incomeSales=commissionRecognizedSalesInRange(range.from,range.to);
  ...
  if($('lowStockRows'))$('lowStockRows').innerHTML=low.slice(0,6).map(p=>`<tr><td><b>${htmlesc(p.code||'')}</b></td><td>${htmlesc(p.name||'')}</td><td><span class="badge red">${visibleStockOf(p.code)}</span></td></tr>`).join('')||'<tr><td colspan="3">Kho ổn định</td></tr>';
}



window.saveCustomer=async()=>{let phone=extractPhone(($('cPhone').value||'').trim());let code=($('cCode').value||customerCodeFromPhone(phone)).trim();let name=cleanCustomerName(($('cName').value||'').trim(),phone,code);let o={customerCode:code,name,type:$('cType').value,phone,address:$('cAddress').value,email:$('cEmail')?.value||'',contact:$('cContact')?.value||'',source:$('cSource')?.value||'',birthday:$('cBirthday')?.value||'',note:$('cNote')?.value||'',discount:+$('cDiscount').value||0,openingDebt:+$('cOpeningDebt').value||0};if(!o.name)return alert('Nhập đúng tên khách hàng, không dùng SĐT làm tên');if(!o.phone)return alert('Nhập số điện thoại khách hàng');if(o.discount<0||o.discount>100)return alert('Chiết khấu khách hàng phải từ 0% đến 100%.');if(o.openingDebt<0)return alert('Công nợ đầu kỳ không được âm.');let id=$('cId').value;if(id){
...
window.editSaleCustomerFromSale=(saleId)=>{
  ...
  const snapshot={id:s.customerId||'', customerCode:ci.code||'', code:ci.code||'', name:ci.name||'', phone:ci.phone||'', address:ci.address||'', type:ci.type||'Khách lẻ', source:ci.source||''};
  document.getElementById('saleCustomerEditModal')?.remove();
...
window.saveSaleCustomerEdit=async()=>{
  ...
  if(!saleId){
    ...
    if($('saleCustomerType')) $('saleCustomerType').value=['Khách lẻ','CTV','Đại lý','Công ty'].includes(type)?type:'Khách lẻ';
    document.getElementById('saleCustomerEditModal')?.remove();
    updateSaleTotals();
...
const CATEGORY_DEFAULTS={
  productCategory:['Khóa thông minh','Khóa cửa gỗ','Khóa cửa nhôm Xingfa','Khóa cổng','Khóa kính','Phụ kiện','Khác'],
...
function categoryOverrideDefaultName(row){return String(row?.defaultName||'').trim()}
function categoryHiddenKeys(type){
  const keys=new Set();
  systemCategoryRows(type).forEach(row=>{
    const defaultName=categoryOverrideDefaultName(row);
    if(defaultName)keys.add(searchKey(defaultName));
    if(row.active==='deleted'&&row.name)keys.add(searchKey(row.name));
  });
  return keys;
}
function systemCategoryNames(type, includeInactive=false){
  const rows=systemCategoryRows(type);
  const hidden=categoryHiddenKeys(type);
  const docs=rows.filter(x=>x.active!=='deleted'&&(includeInactive||x.active!=='inactive')).map(x=>x.name||'').filter(Boolean);
  const defaults=(CATEGORY_DEFAULTS[type]||[]).filter(name=>!hidden.has(searchKey(name)));
  let extra=[];
...
function categoryDocByName(type,name){const k=searchKey(name);return systemCategoryRows(type).find(x=>x.active!=='deleted'&&searchKey(x.name)===k)}
...
function renderSystemCategories(){
  ...
  for(const type of types){
    for(const name of systemCategoryNames(type,true)){
      ...
      const isDefault=!(docRow&&docRow.id)||!!categoryOverrideDefaultName(docRow);
...
window.saveSystemCategory=async()=>{
  ...
  const currentRow=id?(data.systemCategories||[]).find(row=>row.id===id):null;
  const editingDefault=!id&&oldName&&(CATEGORY_DEFAULTS[oldType]||[]).some(x=>searchKey(x)===searchKey(oldName));
  const editingDefaultOverride=!!categoryOverrideDefaultName(currentRow);
  if((editingDefault||editingDefaultOverride)&&type!==oldType)return alert('Danh mục mặc định không thể đổi sang nhóm khác. Bạn có thể đổi tên hoặc tạm ẩn trong cùng nhóm.');
  const exists=systemCategoryNames(type,true).some(x=>searchKey(x)===searchKey(name) && !(oldName&&oldType===type&&searchKey(x)===searchKey(oldName)));
  ...
  else if(editingDefault){await addDoc(col('systemCategories'),{type,name,defaultName:oldName,active,note,createdAt:serverTimestamp(),updatedAt:serverTimestamp()}); await renameCategoryInRelatedData(oldType,oldName,name); await logAction('Tùy chỉnh danh mục mặc định',`${categoryTypeLabel(type)}: ${oldName} -> ${name}`);}
...
window.deleteSystemCategory=async(type,name)=>{
  ...
  if(row?.id&&categoryOverrideDefaultName(row))await updateDoc(doc(db,'systemCategories',row.id),{active:'deleted',updatedAt:serverTimestamp()});
  else if(row?.id)await deleteDoc(doc(db,'systemCategories',row.id));
  else await addDoc(col('systemCategories'),{type,name,defaultName:name,active:'deleted',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
  await logAction('Xóa danh mục',`${categoryTypeLabel(type)}: ${name}`); await loadAll();
};
...
function productCategoryNames(){
  const hidden=categoryHiddenKeys('productCategory');
  const legacy=(data.productCategories||[]).map(x=>x.name||x.category||'').filter(Boolean).filter(name=>!hidden.has(searchKey(name)));
...
window.deleteSelectedProductCategory=async()=>{
  ...
  await deleteSystemCategory('productCategory',name);
}

window.saveProduct=async()=>{let old=$('pId').value?data.products.find(x=>x.id===$('pId').value):{};let o={code:$('pCode').value.trim(),name:$('pName').value,category:$('pCategory').value,cost:has('viewCost')?(+$('pCost').value||0):(+old?.cost||0),price:+$('pPrice').value||0,minStock:+$('pMinStock').value||3,active:($('pActive')?.value||'active')};if(!o.code||!o.name)return alert('Nhập model và tên');if(o.cost<0||o.price<0||o.minStock<0)return alert('Giá vốn, giá bán và tồn tối thiểu không được âm.');let id=$('pId').value;if(id){await updateDoc(doc(db,'products',id),o);await logAction('Sửa sản phẩm',o.code)}else {await addDoc(col('products'),{...o,createdAt:serverTimestamp()});await logAction('Tạo sản phẩm',o.code)}clearProduct();await loadAll()}
...
function renderProducts(){
  ...
  const rows=data.products.filter(p=>{
    ...
    const hay=[p.code,p.name,p.category,p.price,p.cost,visibleStockOf(p.code),(p.active==='inactive'?'ngừng bán':'đang kinh doanh')].join(' ').toLowerCase();
    return !q||hay.includes(q);
  });
  table.innerHTML=rows.map(p=>`<tr><td>${p.code}</td><td>${p.name}</td><td>${p.category||''}</td><td class="view-cost">${money(p.cost)}</td><td>${money(p.price)}</td><td>${visibleStockOf(p.code)}</td><td>${p.active==='inactive'?'<span class="badge red">Ngừng bán</span>':'<span class="badge green">Đang bán</span>'}</td><td><button class="btn ghost" onclick="editProduct('${p.id}')">Sửa</button> <button class="btn danger" onclick="removeDoc('products','${p.id}')">Xóa</button></td></tr>`).join('') || `<tr><td colspan="8">Không tìm thấy sản phẩm phù hợp</td></tr>`;
  applyPermissions();
}
...
window.savePrice=async()=>{
  ...
  if(items.some(x=>x.price<=0))return alert('Giá bán phải lớn hơn 0.');
...
window.saveCostPrice=async()=>{
  ...
  if(items.some(x=>x.cost<=0))return alert('Giá vốn phải lớn hơn 0.');
  if(base.validFrom&&base.validTo&&base.validFrom>base.validTo)return alert('Ngày hiệu lực đến phải lớn hơn hoặc bằng ngày bắt đầu');
...
window.saveStaff=async()=>{let dept=$('eDept').value;let functions=selectedStaffFunctions();if(!functions.length){if(dept==='Sale'||dept==='Quản lý')functions=['Sale'];else if(dept==='Kỹ thuật')functions=['Kỹ thuật'];else if(dept.includes('Kho'))functions=['Kho'];else functions=[dept]}const commissionRaw=$('eCommissionPercent')?.value??'';const techFeeRaw=$('eTechFee')?.value??'';let o={name:$('eName').value.trim(),dept,functions,phone:$('ePhone').value,commissionPercent:commissionRaw===''?5:+commissionRaw,techFee:techFeeRaw===''?100000:+techFeeRaw};if(functions.includes('Sale')||functions.includes('Quản lý')){if(o.commissionPercent<0||o.commissionPercent>100)return alert('Hoa hồng phải nằm trong khoảng 0% đến 100%.')}else{o.commissionPercent=0}if(functions.includes('Kỹ thuật')){if(o.techFee<0)return alert('Công kỹ thuật không được âm.')}else{o.techFee=0}if(!o.name)return alert('Nhập tên nhân viên');let id=$('eId').value;if(id){await updateDoc(doc(db,'staff',id),o);await logAction('Sửa nhân viên',o.name)}else {await addDoc(col('staff'),o);await logAction('Tạo nhân viên',o.name)}$('eId').value='';$('eName').value='';$('ePhone').value='';$('eCommissionPercent').value=5;$('eTechFee').value=100000;$('eDept').value='Sale';setStaffFunctions(['Sale']);await loadAll()}
...
function receiptSaleId(r={}){
  ...
  // debtKey là khóa nghiệp vụ mạnh hơn saleCode hiển thị và không bị ảnh hưởng
  // khi mã phiếu cũ bị trùng hoặc bị sửa.
...
function receiptDedupKey(r={}){
  // Chỉ khử trùng theo định danh chứng từ. Không gộp theo ngày/số tiền/PTTT vì
  // hai lần thu hợp lệ hoàn toàn có thể giống nhau.
  if(r.id)return `receipt-id:${r.id}`;
  if(r.code)return `receipt-code:${r.code}`;
  if(r.sourceDocId)return `receipt-source:${r.sourceDocId}`;
  return '';
}
...
function saleDirectPaid(s={}){
  ...
  if(key && code && key===code && (source==='sale_form' || source==='excel_import' || s.initialPaidAtSale===true || s.directPaidLocked===true)){
...
function saleCollectedInRange(s={},from='',to=''){
  ...
  const grand=+s.grand||0;
  let paid=0;
  const saleDate=reportDateValue(s.directPaidDate||s.date||s.createdDate||s.createdAt||'');
  ...
  saleRefundVouchers(s).forEach(v=>{
    const d=financeDocDate(v);
    if(d && (!from || d>=from) && (!to || d<=to)) paid-=+v.refundAmount||0;
  });
...
function salePaymentInfo(s){
  ...
  // Tương thích dữ liệu cũ: một số phiếu đã được chốt "đã thu đủ" ở các bản trước
  // nhưng chưa có saleId/debtKey trên phiếu thu. Chỉ tin snapshot khi đồng thời có
  // tổng đã thu >= giá trị đơn, công nợ bằng 0 và trạng thái xác nhận đã thanh toán.
  // Không dùng tên khách để phân bổ nên vẫn tránh được lỗi trùng khách hàng.
  const legacyPaid=legacyFullPaymentSnapshot(s,grand);
  const paidBeforeRefund=Math.max(paidAtSale+receiptPaid,legacyPaid);
  const paidTotal=Math.max(0,Math.round(paidBeforeRefund-refunded));
...
function legacyFullPaymentSnapshot(s={},grandValue=0){
  const grand=Math.max(0,Math.round(+grandValue||+s.grand||0));
  if(grand<=0)return 0;
  const hasDebtSnapshot=['debtLeft','debt'].some(k=>s[k]!==undefined&&s[k]!==null&&String(s[k])!=='');
  const debtValues=['debtLeft','debt'].filter(k=>s[k]!==undefined&&s[k]!==null&&String(s[k])!=='').map(k=>Math.round(+s[k]||0));
  const debtCleared=hasDebtSnapshot&&debtValues.every(v=>v<=0);
  const storedPaid=Math.max(0,...['paidTotal','totalPaid','collectedAmount','amountPaid','paid'].map(k=>Math.round(+s[k]||0)));
  const status=searchKey([s.paymentStatus,s.status,s.moneyStatus].filter(Boolean).join(' '));
  const paidStatus=['da thu','da thanh toan','paid','completed','hoan tat'].some(x=>status.includes(x));
  const commissionAlreadyEarned=String(s.commissionStatus||'').toLowerCase()==='earned'||!!s.commissionEarnedAt;
  return storedPaid>=grand&&debtCleared&&(paidStatus||commissionAlreadyEarned)?grand:0;
}
function saleFullyPaidForCommission(s={}){
  ...
  // Ưu tiên khoản thu trực tiếp có khóa + phiếu thu còn hiệu lực - tiền hoàn.
  // Dữ liệu cũ chỉ được nhận khi snapshot đồng thời đủ tiền, hết nợ và đã chốt thu.
...
function salePaymentEvents(s={}){
  ...
  if(direct>0)events.push({date:reportDateValue(s.directPaidDate||s.date||s.createdDate||s.createdAt||''),amount:direct,type:'sale_direct',id:s.id||s.code||''});
  receiptsForSalePayment(s).forEach(r=>events.push({date:financeDocDate(r),amount:+r.amount||0,type:'receipt',id:r.id||r.code||''}));
  saleRefundVouchers(s).forEach(v=>events.push({date:financeDocDate(v),amount:-(+v.refundAmount||0),type:'refund',id:v.id||v.code||''}));
  const valid=events.filter(e=>e.date&&e.amount).sort((a,b)=>String(a.date).localeCompare(String(b.date))||String(a.type).localeCompare(String(b.type))||String(a.id).localeCompare(String(b.id)));
  const grand=Math.max(0,Math.round(+s.grand||0));
  const eventPaid=Math.max(0,Math.round(valid.reduce((a,e)=>a+(+e.amount||0),0)));
  // Nếu dữ liệu cũ đủ bằng chứng đã thu đủ nhưng thiếu liên kết chứng từ, bổ sung
  // đúng phần còn thiếu tại ngày đã chốt. Nhờ vậy đơn tháng 7 không bị rơi sang
  // tháng hiện tại chỉ vì được mở/chỉnh sửa lại sau đó.
  if(grand>0&&eventPaid<grand&&legacyFullPaymentSnapshot(s,grand)>=grand){
    const legacyDate=reportDateValue(s.commissionEarnedAt||s.fullyPaidAt||s.paymentCompletedDate||s.settledDate||s.directPaidDate||s.date||s.createdDate||s.createdAt||'');
    if(legacyDate)valid.push({date:legacyDate,amount:grand-eventPaid,type:'legacy_paid_snapshot',id:s.id||s.code||''});
  }
  return valid.sort((a,b)=>String(a.date).localeCompare(String(b.date))||String(a.type).localeCompare(String(b.type))||String(a.id).localeCompare(String(b.id)));
}
function saleCommissionEarnedAt(s={}){
  ...
  earnedAt=earnedAt||reportDateValue(s.commissionEarnedAt||s.fullyPaidAt||s.paymentCompletedDate||s.settledDate||s.directPaidDate||s.date||'');
  // Chỉ dùng ngày chặn khi có lý do nghiệp vụ rõ ràng (giảm giá trị đơn hoặc trả hàng
  // làm số tiền đã thu trở thành đủ). Dữ liệu cũ chỉ có ngày chặn nhưng không có lý do
  // từng làm đơn tháng 7 bị đẩy sang tháng 8, nên không còn được áp dụng mù quáng.
  const eligibilityFloor=reportDateValue(s.commissionEligibilityDate||'');
  const eligibilityReason=String(s.commissionEligibilityReason||'');
  const trustedEligibilityFloor=['order_edit','sale_return'].includes(eligibilityReason)||s.hasReturn===true||(+s.returnedQty||0)>0;
...
function saleIsInstalled(s={}){
  const st=searchKey(inferSaleInstallStatus(s));
  return st==='da lap'||st.includes('da lap')||st.includes('hoan tat')||st.includes('hoan thanh');
}
function saleInstallationCompletedAt(s={}){
  if(!saleIsInstalled(s))return '';
  const explicit=String(s.installCompletedDate||s.installCompleteDate||s.completedInstallDate||s.installedAt||s.installDate||'').slice(0,10);
  if(explicit)return explicit;
  const voucher=stockVoucherForSale(s);
  return String(voucher?.date||s.date||'').slice(0,10);
}
function saleRevenueRecognitionDate(s={}){
  // V112: Tổng doanh thu chỉ phát sinh khi các khoản thu gắn đúng phiếu bán
  // đã làm phiếu đó đạt đủ 100%. Trạng thái lắp đặt không làm thay đổi doanh thu.
  // Ngày ghi nhận chính là ngày khoản thu làm số tiền lũy kế đạt 100%.
  if(isSaleCanceled(s)||!saleFullyPaidForCommission(s))return '';
  const fullyPaidAt=saleCommissionEarnedAt(s);
  return fullyPaidAt||'';
}
function revenueRecognizedSalesInRange(from='',to=''){
...
function saleMoneyStatus(s){
  ...
  if(s.returnStatus==='Đã trả hết'||(+s.returnedQty>0&&grand<=0))return {overPaid:over,label:over>0?(s.returnSettlement||'Cần xử lý tiền dư'):'Đã trả hết hàng',badge:over>0?'orange':'gray'};
...
async function updatePaymentStatusesForCustomer(customerId){
  ...
  for(const s of rows){
    ...
    const wasEarned=String(s.commissionStatus||'')==='earned'||!!s.commissionEarnedAt;
    const isEarned=!!earnedAt;
    const expectedSaleCommission=saleExpectedCommissionValue(s);
    const history=Array.isArray(s.commissionHistory)?s.commissionHistory.slice():[];
    if(wasEarned!==isEarned){
      history.push({type:isEarned?'earned':'reversed',date:isEarned?earnedAt:today(),amount:isEarned?expectedSaleCommission:-Math.abs(+s.saleCommission||expectedSaleCommission||0),reason:'Cập nhật thu tiền/công nợ',at:new Date().toISOString()});
    }
    try{await updateDoc(doc(db,'sales',s.id),{paidTotal:st.paidTotal,debt:st.debtLeft,debtLeft:st.debtLeft,paymentStatus:st.paymentStatus,status:st.paymentStatus,expectedSaleCommission,saleCommission:isEarned?expectedSaleCommission:0,commissionEarnedAt:earnedAt,commissionStatus:isEarned?'earned':(wasEarned?'reversed':'pending'),commissionHistory:history,updatedAt:serverTimestamp()});}catch(e){console.warn('Không cập nhật trạng thái công nợ đơn '+s.id,e.message)}
  }
}
async function updatePaymentStatusForSaleSnapshot(saleId){
  ...
  try{await updateDoc(doc(db,'sales',saleId),{paidTotal:pay.paidTotal,debt:pay.debtLeft,debtLeft:pay.debtLeft,paymentStatus:pay.paymentStatus,status:pay.paymentStatus,expectedSaleCommission,saleCommission:isEarned?expectedSaleCommission:0,commissionEarnedAt:earnedAt,commissionStatus:isEarned?'earned':(wasEarned?'reversed':'pending'),commissionHistory:history,updatedAt:serverTimestamp()});}catch(e){console.warn('Không cập nhật công nợ phiếu '+saleId,e.message)}
}
...
function saleRefundVouchers(s){return saleReturnVouchers(s).filter(v=>v.settlement==='Đã hoàn tiền'&&(+v.refundAmount||0)>0)}
function returnVoucherItemAmount(v={},it={}){
  const items=v.items||[];
  const totalLine=items.reduce((a,x)=>a+(+x.lineAmount||lineNet(x)||0),0);
  const voucherAmount=+v.refundAmount||0;
  const line=+it.lineAmount||lineNet(it)||0;
  if(voucherAmount>0&&totalLine>0)return voucherAmount*line/totalLine;
  return line;
}
...
function calcSaleFromItemsForReturn(s,items){
  const remainingBeforeOrderDiscount=items.reduce((a,it)=>a+lineNet(it),0);
  let orderDiscountType=s.orderDiscountType||'none';
  let orderDiscountValue=+s.orderDiscountValue||0;
  // Chiết khấu số tiền được phân bổ theo tỷ lệ hàng còn lại; chiết khấu % giữ nguyên tỷ lệ.
  if(['fixed','amount'].includes(orderDiscountType)){
    const originalBefore=+s.subtotalBeforeOrderDiscount||((s.items||[]).reduce((a,it)=>a+lineNet(it),0));
    const originalDiscount=+s.orderDiscountTotal||orderDiscountValue;
    orderDiscountValue=originalBefore>0?originalDiscount*remainingBeforeOrderDiscount/originalBefore:0;
  }
  const surcharge=items.length?(+s.surcharge||0):0;
  const totals=calcSaleTotals(items,s.vatMode||'included8',salePaymentInfo(s).paidTotal,surcharge,orderDiscountType,orderDiscountValue);
  ...
  const techCost=items.length?(+s.techCost||0):0;
  const techFuel=items.length?(+s.techFuel||0):0;
...
window.openSaleReturn=id=>{
  ...
  <div class="grid form-grid"><div><label>Ngày trả</label><input id="returnDate" type="date" value="${today()}"></div><div><label>Nhập về kho</label><select id="returnWarehouse">${warehouseOptions(wh)}</select></div><div><label>Xử lý tiền dư nếu đã thu đủ</label><select id="returnSettlement"><option>Khách đang dư tiền</option><option>Cần hoàn tiền</option><option>Đã hoàn tiền</option><option>Cấn trừ đơn sau</option></select></div><div><label>Phương thức hoàn tiền</label><select id="returnPaymentMethod">${PAYMENT_METHODS.map(x=>`<option>${x}</option>`).join('')}</select></div><div class="span2"><label>Ghi chú</label><input id="returnNote" value="Khách chỉ nhận một phần, nhập lại hàng dư về kho"></div></div>
  <table class="editable"><thead><tr><th>Model</th><th>Tên sản phẩm</th><th>SL còn tính bán</th><th>Đã trả trước</th><th>SL trả lần này</th></tr></thead><tbody id="returnItems">${rows.map(it=>`<tr data-code="${it.code}" data-name="${it.name||''}" data-max="${+it.qty||0}" data-price="${+it.price||0}" data-discount-type="${it.discountType||'percent'}" data-discount="${+it.discount||0}"><td><b>${it.code}</b></td><td>${it.name||''}</td><td>${it.qty}</td><td>${returned[it.code]||0}</td><td><input type="number" min="0" max="${+it.qty||0}" value="0"></td></tr>`).join('')}</tbody></table>
...
window.saveSaleReturn=async(id)=>{
  ...
  for(const tr of rows){
    const code=tr.dataset.code,name=tr.dataset.name,max=+tr.dataset.max||0,price=+tr.dataset.price||0,discountType=tr.dataset.discountType||'percent',discount=+tr.dataset.discount||0;
    ...
    if(qty>0){const p=data.products.find(x=>x.code===code)||{}; const saleLine={code,name,qty,price,discountType,discount};returnItems.push({...saleLine,inputQty:qty,cost:+p.cost||costFor(code,s.date||today())||0,lineAmount:lineNet(saleLine),note:`Trả lại từ đơn bán ${s.code}`}); returnMap[code]=qty;}
  }
  ...
  const returnDate=$('returnDate')?.value||today();
  const refundAmount=Math.min(Math.max(0,(+s.grand||0)-(+newTotals.grand||0)),+oldPay.paidTotal||0);
  const refundPaid=returnSettlement==='Đã hoàn tiền'?refundAmount:0;
  const overPaid=Math.max(0,(+oldPay.paidTotal||0)-refundPaid-(+newTotals.grand||0));
  const becameFullByReturn=(+s.grand||0)>(+oldPay.paidTotal||0)&&(+newTotals.grand||0)>0&&((+oldPay.paidTotal||0)-refundPaid)>=(+newTotals.grand||0);
  ...
  const newPaidAfterReturn=Math.max(0,(+oldPay.paidTotal||0)-refundPaid);
  const newCommissionEarned=(+newTotals.grand||0)>0&&newPaidAfterReturn>=(+newTotals.grand||0);
  const commissionHistory=Array.isArray(s.commissionHistory)?s.commissionHistory.slice():[];
  if(saleFullyPaidForCommission(s)&&oldCommission!==newCommission)commissionHistory.push({type:'adjustment',date:returnDate,amount:newCommission-oldCommission,reason:`Trả hàng ${voucher.code}`,at:new Date().toISOString()});
  await updateDoc(doc(db,'sales',s.id),{items:newItems,...newTotals,expectedSaleCommission:newCommission,saleCommission:newCommissionEarned?newCommission:0,returnedQty,hasReturn:true,returnStatus:newItems.length?'Đã trả một phần':'Đã trả hết',overPaid,returnSettlement,moneyStatus:overPaid>0?returnSettlement:'',commissionEligibilityDate,commissionEligibilityReason,commissionHistory,updatedAt:serverTimestamp()});
  await logAction('Trả lại hàng bán',`${voucher.code} - đơn ${s.code} - giá trị giảm ${money(refundAmount)}${refundPaid?' - đã hoàn tiền':''}`);
  document.getElementById('saleReturnModal')?.remove();
  document.getElementById('saleDetailModal')?.remove();
  await loadAll();
  await updatePaymentStatusForSaleSnapshot(s.id);
  await loadAll();
...
window.saleInstallStatusChanged=()=>{if($('saleInstallStatus')?.value==='Đã lắp'&&!$('saleInstallCompletedDate')?.value)$('saleInstallCompletedDate').value=today();};
...
window.saveSale=async()=>{let customer=findCustomerBySearch();if(!customer){quickCreateCustomer();return alert('Chưa có khách hàng hợp lệ. Vui lòng kiểm tra Tên/SĐT rồi bấm Lưu khách trong form Thêm khách hàng.')}let items=saleItems();if(!items.length)return alert('Chưa có sản phẩm'); const inactiveItem=items.find(it=>(data.products.find(p=>p.code===it.code)||{}).active==='inactive'); if(inactiveItem)return alert('Model '+inactiveItem.code+' đang ngừng bán, không thể lưu phiếu bán.');
  if(items.some(it=>(+it.qty||0)<=0||(+it.price||0)<0))return alert('Số lượng phải lớn hơn 0 và đơn giá không được âm.');
  const rawSurcharge=+($('saleSurcharge')?.value||0)||0;
  const rawTechCost=+($('saleTechCost')?.value||0)||0;
  const rawTechFuel=+($('saleTechFuel')?.value||0)||0;
  const rawCommissionPercent=+($('saleCommissionPercent')?.value||0)||0;
  const rawInitialPaid=+($('salePaid')?.value||0)||0;
  if(rawSurcharge<0||rawTechCost<0||rawTechFuel<0)return alert('Phụ thu, công kỹ thuật và tiền xăng không được âm.');
  if(rawCommissionPercent<0||rawCommissionPercent>100)return alert('Tỷ lệ hoa hồng phải nằm trong khoảng 0% đến 100%.');
  if(!editingSale&&rawInitialPaid<0)return alert('Số tiền đã thu không được âm.');
  ...
  let totals=calcSaleTotals(items,$('saleVatMode').value,paidForTotals,rawSurcharge,$('saleOrderDiscountType')?.value||'none',$('saleOrderDiscountValue')?.value||0);if(!editingSale&&paidForTotals>totals.grand)return alert('Số tiền thu trực tiếp không được lớn hơn tổng tiền đơn. Nếu khách thanh toán bổ sung, hãy lập Phiếu thu theo đúng đơn.');let installCompletedDate=($('saleInstallCompletedDate')?.value||'').slice(0,10);let installStatus=$('saleInstallStatus')?.value||((installCompletedDate||exportStock)?'Đã lắp':'Chưa lắp');if(installStatus==='Đã lắp'&&!installCompletedDate)installCompletedDate=editingSale?today():saleDateVal;let cost=items.reduce((a,it)=>a+costFor(it.code,saleDateVal)*it.qty,0);let commissionPercent=rawCommissionPercent;let expectedSaleCommission=calcCommission(totals,commissionPercent);let techCost=rawTechCost;let techFuel=rawTechFuel;let commissionBase=calcCommissionBase(totals);const editingEarned=!!editingSale&&saleFullyPaidForCommission(oldSale);const earnedAfterSave=totals.grand>0&&paidForTotals>=totals.grand;const initiallyEarned=!editingSale&&earnedAfterSave;const becameFullByEdit=!!editingSale&&!editingEarned&&earnedAfterSave;const commissionEligibilityDate=becameFullByEdit?today():(oldSale?.commissionEligibilityDate||'');const commissionEligibilityReason=becameFullByEdit?'order_edit':(oldSale?.commissionEligibilityReason||'');const earnedAtForSave=earnedAfterSave?(editingSale?(editingEarned?(oldSale?.commissionEarnedAt||saleCommissionEarnedAt(oldSale)||today()):today()):saleDateVal):'';const commissionHistory=editingSale?(Array.isArray(oldSale?.commissionHistory)?oldSale.commissionHistory.slice():[]):(initiallyEarned?[{type:'earned',date:saleDateVal,amount:expectedSaleCommission,reason:'Thu đủ trực tiếp khi tạo đơn',at:new Date().toISOString()}]:[]);let snap=customerSnapshotFromCustomer(customer,saleCustomerType()); snap.phone=saleCustomerPhoneValue(customer)||snap.phone; snap.address=saleCustomerAddressValue(customer); let o={code:$('saleCode').value,date:saleDateVal,...customerSnapshotPayload(snap),staffId:$('saleStaff').value,staffName:data.staff.find(x=>x.id===$('saleStaff').value)?.name||'',techId:$('saleTech').value,techName:data.staff.find(x=>x.id===$('saleTech').value)?.name||'',commissionPercent,commissionBase,expectedSaleCommission,saleCommission:earnedAfterSave?expectedSaleCommission:0,techCost,techFuel,vatMode:$('saleVatMode').value,paid:editingSale?(+(oldSale?.paid||0)||0):paidForTotals,paidSource:editingSale?(oldSale?.paidSource||''):(paidForTotals>0?'sale_form':''),paidEntryKey:editingSale?(oldSale?.paidEntryKey||oldSale?.paidSaleCode||''):(paidForTotals>0?$('saleCode').value:''),directPaidDate:editingSale?(oldSale?.directPaidDate||oldSale?.date||saleDateVal):(paidForTotals>0?saleDateVal:''),paymentMethod:editingSale?(oldSale?.paymentMethod||$('salePaymentMethod')?.value||'Tiền mặt'):($('salePaymentMethod')?.value||'Tiền mặt'),directPaidLocked:editingSale?(oldSale?.directPaidLocked===true):(paidForTotals>0),note:$('saleNote').value,items,...totals,cost,profit:commissionBase-cost-expectedSaleCommission-techCost-techFuel,status:totals.debt>0?(paidForTotals>0?'Thanh toán một phần':'Chưa thu tiền'):'Đã thu tiền',paymentStatus:totals.debt>0?(paidForTotals>0?'Thanh toán một phần':'Chưa thu tiền'):'Đã thu tiền',paidTotal:paidForTotals,debtLeft:totals.debt,commissionEarnedAt:earnedAtForSave,commissionStatus:earnedAfterSave?'earned':(editingEarned?'reversed':'pending'),commissionEligibilityDate,commissionEligibilityReason,commissionHistory,installStatus,installCompletedDate,warehouse:saleWarehouse,stockExported:exportStock,stockVoucherId:oldSale?.stockVoucherId||'',updatedAt:serverTimestamp()};
  if(editingSale&&!editingEarned&&earnedAfterSave)commissionHistory.push({type:'earned',date:earnedAtForSave,amount:expectedSaleCommission,reason:'Sửa giá trị đơn làm số đã thu đạt đủ 100%',at:new Date().toISOString()});
  else if(editingSale&&editingEarned&&!earnedAfterSave)commissionHistory.push({type:'reversed',date:today(),amount:-saleExpectedCommissionValue(oldSale),reason:'Sửa giá trị đơn làm số đã thu không còn đủ 100%',at:new Date().toISOString()});
  else if(editingSale&&editingEarned&&earnedAfterSave&&saleExpectedCommissionValue(oldSale)!==expectedSaleCommission)commissionHistory.push({type:'adjustment',date:today(),amount:expectedSaleCommission-saleExpectedCommissionValue(oldSale),reason:'Sửa dữ liệu tài chính đơn hàng',at:new Date().toISOString()});
...
function periodRange(period,referenceDate=today(),selectedMonth=''){
  const ref=isoLocalDate(referenceDate||today());
  const iso=isoDateFromLocal;
  if(period==='day') return {from:iso(ref),to:iso(ref)};
  if(period==='week'){
  ...
  if(period==='month'){
    const validMonth=/^\d{4}-(0[1-9]|1[0-2])$/.test(String(selectedMonth||''));
    const month=validMonth?String(selectedMonth):`${ref.getFullYear()}-${String(ref.getMonth()+1).padStart(2,'0')}`;
    const [y,m]=month.split('-').map(Number);
    return {from:`${month}-01`,to:iso(new Date(y,m,0))};
  }
  if(period==='year'){
...
function readCommissionFilterFromForm(){
  ...
  let selectedMonth=$('commissionMonth')?.value||'';
  if(period==='month'&&!selectedMonth){selectedMonth=monthKey(today());if($('commissionMonth'))$('commissionMonth').value=selectedMonth;}
  const range=period==='custom'?{from:$('commissionFrom')?.value||'',to:$('commissionTo')?.value||''}:periodRange(period,today(),selectedMonth);
  if(range.from&&range.to&&range.from>range.to){const old=range.from;range.from=range.to;range.to=old;}
  if(period!=='custom'){
...
window.refreshCommissionStaffOptions=()=>{
  ...
  let list=data.staff.filter(x=>!dept||staffHasFunction(x,dept)||(dept==='Sale'&&staffHasFunction(x,'Quản lý')));
  if(!canViewAllCommissions()){
    const own=currentStaffMatchIds();
...
window.commissionPeriodChanged=()=>{
  ...
  if(period!=='custom'){
    let selectedMonth=$('commissionMonth')?.value||'';
    if(period==='month'&&!selectedMonth){selectedMonth=monthKey(today());if($('commissionMonth'))$('commissionMonth').value=selectedMonth;}
    const range=periodRange(period,today(),selectedMonth);
    if($('commissionFrom')) $('commissionFrom').value=range.from;
    if($('commissionTo')) $('commissionTo').value=range.to;
  }
};
window.commissionMonthChanged=()=>{
  const selectedMonth=$('commissionMonth')?.value||'';
  if(!selectedMonth)return;
  if($('commissionPeriod'))$('commissionPeriod').value='month';
  const range=periodRange('month',today(),selectedMonth);
  if($('commissionFrom'))$('commissionFrom').value=range.from;
  if($('commissionTo'))$('commissionTo').value=range.to;
...
window.resetCommissionFilter=()=>{
  ...
  if($('commissionMonth'))$('commissionMonth').value='';
  if($('commissionFrom'))$('commissionFrom').value='';
  if($('commissionTo'))$('commissionTo').value='';
  commissionAppliedFilter={q:'',dept:'',staffId:'',period:'all',month:'',from:'',to:''};
  renderCommissions();
};
...
function staffByIdentity(id='',name=''){
  const direct=id?data.staff.find(x=>x.id===id):null;
  if(direct)return direct;
  const key=searchKey(name||'');
  if(!key)return null;
  const matches=data.staff.filter(x=>searchKey(x.name||'')===key);
  return matches.length===1?matches[0]:null;
}
function commissionStaffMeta(s={},role='Sale'){
  const isSale=role==='Sale';
  const rawId=String(isSale?s.staffId||'':s.techId||'').trim();
  const rawName=String(isSale?s.staffName||'':s.techName||'').trim();
  const staff=staffByIdentity(rawId,rawName);
  return {id:staff?.id||rawId||rawName||'none',name:rawName||staff?.name||(isSale?'Chưa chọn sale':'Chưa chọn kỹ thuật'),staff};
}
function commissionRoleIncluded(s={},role='Sale',filter=commissionAppliedFilter||{}){
  const f=filter||{};
  if(f.dept&&f.dept!==role)return false;
  const meta=commissionStaffMeta(s,role);
  if(f.staffId&&meta.id!==f.staffId)return false;
  if((f.dept||f.staffId)&&meta.id==='none')return false;
  const q=searchKey(f.q||'');
  if(q){
    const ci=saleCustomerInfo(s);
    const orderText=searchKey([s.code,ci.name,ci.phone,ci.code,s.customerName,s.customerPhone,s.customerCode,saleItemSummary(s).models].join(' '));
    if(!orderText.includes(q)&&!searchKey(meta.name).includes(q))return false;
  }
  return true;
}
function commissionPartsForFilter(s={},filter=commissionAppliedFilter||{}){
  const includeSale=commissionRoleIncluded(s,'Sale',filter);
  const includeTech=commissionRoleIncluded(s,'Kỹ thuật',filter);
  return {includeSale,includeTech,saleCommission:includeSale?saleCommissionValue(s):0,techCost:includeTech?(+s.techCost||0):0,techFuel:includeTech?(+s.techFuel||0):0};
}
function commissionFilteredSales(){
  ...
  return commissionEligibleSales().filter(s=>{
    ...
    return parts.includeSale||parts.includeTech;
  });
}
...
function salaryBonusDeductByStaff(from='',to='',staffId='',q='',dept=''){
  const m={};
  data.salaries.forEach(x=>{
    const d=reportDateValue(x.date||x.salaryDate||x.createdDate||'');
    if(!d||(from&&d<from)||(to&&d>to))return;
    const staff=staffByIdentity(x.staffId||'',x.staffName||'');
    const id=staff?.id||x.staffId||x.staffName||'none';
    const name=x.staffName||staff?.name||'Chưa chọn';
    if(staffId&&id!==staffId)return;
    if(dept&&(!staff||!staffHasFunction(staff,dept)))return;
    if(q&&!searchKey(name).includes(searchKey(q)))return;
    m[id]=m[id]||{id,name,bonus:0,deduct:0};
    m[id].bonus+=+x.bonus||0;
    m[id].deduct+=+x.deduct||0;
  });
  return m;
}
function employeeIncomeRows(){
  const f=commissionAppliedFilter||{},sales=commissionFilteredSales(),map={};
  const add=(id,name)=>{id=id||name||'none';map[id]=map[id]||{id,name:name||'Chưa chọn',saleCommission:0,techCost:0,techFuel:0,bonus:0,deduct:0,total:0};return map[id];};
  sales.forEach(s=>{
    const parts=commissionPartsForFilter(s,f),saleMeta=commissionStaffMeta(s,'Sale'),techMeta=commissionStaffMeta(s,'Kỹ thuật');
    if(parts.includeSale)add(saleMeta.id,saleMeta.name).saleCommission+=parts.saleCommission;
    if(parts.includeTech){const t=add(techMeta.id,techMeta.name);t.techCost+=parts.techCost;t.techFuel+=parts.techFuel;}
  });
  const bd=salaryBonusDeductByStaff(f.from,f.to,f.staffId||'',f.q||'',f.dept||'');
  Object.values(bd).forEach(v=>{const r=add(v.id,v.name);r.bonus+=v.bonus;r.deduct+=v.deduct;});
...
function techPerformanceRows(){
  const rows=commissionFilteredSales(),f=commissionAppliedFilter||{},map={};
  if(f.dept==='Sale')return [];
  rows.forEach(s=>{const parts=commissionPartsForFilter(s,{...f,dept:'Kỹ thuật'});if(!parts.includeTech)return;const meta=commissionStaffMeta(s,'Kỹ thuật'),id=meta.id;map[id]=map[id]||{id,name:meta.name,count:0,qty:0,techCost:0,techFuel:0,warranty:0};map[id].count++;map[id].qty+=(s.items||[]).reduce((a,it)=>a+(+it.qty||0),0);map[id].techCost+=parts.techCost;map[id].techFuel+=parts.techFuel;});
  activeWarranties().forEach(w=>{const d=warrantyReportDate(w);if(f.from&&d<f.from)return;if(f.to&&d>f.to)return;const staff=staffByIdentity(w.techId||'',w.techName||''),id=staff?.id||w.techId||w.techName||'';if(f.staffId&&id!==f.staffId)return;if(f.q&&!searchKey([w.techName,w.code,w.saleCode,w.customer].join(' ')).includes(searchKey(f.q)))return;if(!id)return;map[id]=map[id]||{id,name:w.techName||staff?.name||'Kỹ thuật',count:0,qty:0,techCost:0,techFuel:0,warranty:0};map[id].warranty++;});
  return Object.values(map).sort((a,b)=>b.count-a.count||b.qty-a.qty);
}
function renderCommissions(){
  if(!$('commissionByStaff')||!$('commissionByOrder'))return;

  const rows=commissionFilteredSales();
  const applied=commissionAppliedFilter||{};
  const bySale={};
  const byTech={};
  ...
  rows.forEach(s=>{
    const parts=commissionPartsForFilter(s,applied);
    const commissionBase=saleCommissionBaseValue(s);
    const saleCommissionAmt=parts.saleCommission;
    totalRevenue+=+s.grand||0;
    if(parts.includeSale){totalCommissionBase+=commissionBase;totalSurcharge+=+s.surcharge||0;totalDiscount+=+s.discountTotal||0;}
    totalSaleCommission+=saleCommissionAmt;
    totalTechCost+=parts.techCost;
    totalTechFuel+=parts.techFuel;
    if(parts.includeSale){
      const saleMeta=commissionStaffMeta(s,'Sale'),saleKey=saleMeta.id;
      bySale[saleKey]=bySale[saleKey]||{id:saleKey,name:saleMeta.name,count:0,revenue:0,commissionBase:0,surcharge:0,discount:0,commission:0};
    ...
    if(parts.includeTech){
      const techMeta=commissionStaffMeta(s,'Kỹ thuật'),techKey=techMeta.id;
      byTech[techKey]=byTech[techKey]||{name:techMeta.name,count:0,revenue:0,techCost:0,techFuel:0};
      byTech[techKey].count++;
      byTech[techKey].revenue+=+s.grand||0;
      byTech[techKey].techCost+=parts.techCost;
      byTech[techKey].techFuel+=parts.techFuel;
    }
  });
...
window.saveSalary=async()=>{
  ...
  if([base,allowance,bonus,deduct].some(x=>x<0)||total<0)return alert('Các khoản lương, phụ cấp, thưởng và khấu trừ không được âm.');
...
function receiptDebtKey(r={}){
  ...
  if(r.debtKey && String(r.debtKey).startsWith('opening:')) return String(r.debtKey);
...
function debtSettledDate(d){
  const sale=d?.sales?.[0];
  if(sale)return saleCommissionEarnedAt(sale);
  const total=+d?.total||0;
  let running=0,settledAt='';
  uniqueReceiptsForFinance(d?.receipts||[]).map(r=>({date:financeDocDate(r),amount:+r.amount||0,id:r.id||r.code||''})).filter(x=>x.date&&x.amount).sort((a,b)=>String(a.date).localeCompare(String(b.date))||String(a.id).localeCompare(String(b.id))).forEach(x=>{
    const before=running;
    running=Math.max(0,running+x.amount);
    if(before<total&&running>=total)settledAt=x.date;
    if(running<total)settledAt='';
  });
  return settledAt;
}
...
window.printReceipt=id=>{
  ...
  let html=`<div class="print-a5 receipt-a5">
    <div class="receipt-brand">
      <div><b>SIMILOCK ĐÀ NẴNG</b><br><span>Đ/c: 223 Trường Chinh, P. An Khê, TP. Đà Nẵng</span><br><span>CN HCM: 403 Nguyễn Thái Bình, P. Bảy Hiền, TP.HCM</span><br><span>Hotline: 0902.950.816</span></div>
...
function cashbookRows(from='',to=''){
  ...
  activeSales().forEach(s=>{
    ...
    const date=financeDocDate({date:s.directPaidDate||s.date,createdDate:s.createdDate,createdAt:s.createdAt}); if(!date)return;
    const ci=saleCustomerInfo(s);
    pushRow({
  ...
  // 3) Tiền hoàn khách do trả hàng, chỉ ghi sổ quỹ khi đã xác nhận "Đã hoàn tiền".
  activeStockVouchers().filter(v=>v.type==='RETURN'&&v.settlement==='Đã hoàn tiền'&&(+v.refundAmount||0)>0).forEach(v=>{
    const date=financeDocDate(v);if(!date)return;
    pushRow({id:v.id||'',date,code:v.code||'',type:'Chi',content:`Hoàn tiền trả hàng ${v.saleCode||''}${v.customerName?' - '+v.customerName:''}`.trim(),paymentMethod:normalizePaymentMethod(v.paymentMethod)||'Tiền mặt',income:0,expense:+v.refundAmount||0,source:'return_refund'});
  });

  // 4) Phiếu chi/chi phí. Chỉ lấy chứng từ có ngày hợp lệ.
...
window.printCashbook=()=>{
  ...
  const html=`<div class="print-a5"><div style="text-align:center"><b>SIMILOCK ĐÀ NẴNG</b><br>Đ/c: 223 Trường Chinh, P. An Khê, TP. Đà Nẵng<br>Hotline: 0902.950.816<h2>SỔ QUỸ</h2><div>Từ ${from} đến ${to}</div></div><table><thead><tr><th>Ngày</th><th>Chứng từ</th><th>Nội dung</th><th>Thu</th><th>Chi</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r.date}</td><td>${r.code}</td><td>${htmlesc(r.content)}<br><small>${r.paymentMethod}</small></td><td>${r.income?money(r.income):''}</td><td>${r.expense?money(r.expense):''}</td></tr>`).join('')}</tbody></table><p><b>Số dư đầu kỳ:</b> ${money(opening)}<br><b>Tổng thu theo phiếu thu:</b> ${money(income)}<br><b>Tổng chi trong kỳ:</b> ${money(expense)}<br><b>Số dư cuối kỳ:</b> ${money(closing)}<br><small>Số dư cuối kỳ = số dư đầu kỳ + thu - chi. Tiền vào quỹ trên Dashboard dùng cùng nguồn với Tổng thu sổ quỹ; Thu theo đơn là chỉ số quản trị bán hàng nên có thể khác khi thu công nợ cũ.</small></p><div style="display:flex;justify-content:space-between;text-align:center;margin-top:30px"><div>Người lập<br><br><br></div><div>Kế toán<br><br><br></div><div>Quản lý<br><br><br></div></div></div>`;
  doPrint(html);
}
function warehouseUsage(name){
  const voucherCount=(data.stockVouchers||[]).filter(v=>[v.warehouse,v.fromWarehouse,v.toWarehouse].includes(name)).length;
  const saleCount=(data.sales||[]).filter(s=>s.warehouse===name).length;
  const userCount=(data.users||[]).filter(u=>Array.isArray(u.warehouseAccess)&&u.warehouseAccess.includes(name)).length;
  return {voucherCount,saleCount,userCount,total:voucherCount+saleCount+userCount};
}
async function renameWarehouseInRelatedData(oldName,newName){
  if(!oldName||oldName===newName)return;
  for(const v of (data.stockVouchers||[])){
    const patch={};
    if(v.warehouse===oldName)patch.warehouse=newName;
    if(v.fromWarehouse===oldName)patch.fromWarehouse=newName;
    if(v.toWarehouse===oldName)patch.toWarehouse=newName;
    if(Object.keys(patch).length){patch.updatedAt=serverTimestamp();await updateDoc(doc(db,'stockVouchers',v.id),patch);}
  }
  for(const sale of (data.sales||[]).filter(s=>s.warehouse===oldName))await updateDoc(doc(db,'sales',sale.id),{warehouse:newName,updatedAt:serverTimestamp()});
  for(const user of (data.users||[]).filter(u=>Array.isArray(u.warehouseAccess)&&u.warehouseAccess.includes(oldName))){
    const warehouseAccess=[...new Set(user.warehouseAccess.map(name=>name===oldName?newName:name))];
    await updateDoc(doc(db,'users',user.id),{warehouseAccess,updatedAt:serverTimestamp()});
  }
  if(Array.isArray(currentPerm.warehouseAccess))currentPerm.warehouseAccess=currentPerm.warehouseAccess.map(name=>name===oldName?newName:name);
}
function renderWarehouseMaster(){
  const warehouseTable=$('warehouseMasterTable');
  if(warehouseTable){
    const rows=warehouseMasterRows(true);
    warehouseTable.innerHTML=rows.map(row=>{
      const usage=warehouseUsage(row.name);
      const protectedWarehouse=PROTECTED_WAREHOUSE_NAMES.includes(row.name);
      return `<tr><td><b>${htmlesc(row.code||'')}</b></td><td><b>${htmlesc(row.name||'')}</b>${protectedWarehouse?'<br><small>Kho hệ thống được bảo vệ</small>':''}</td><td>${htmlesc(row.address||'')}</td><td>${htmlesc(row.phone||'')}</td><td>${row.active==='inactive'?'<span class="badge orange">Tạm ẩn</span>':'<span class="badge green">Đang dùng</span>'}</td><td>${usage.total}<br><small>${usage.voucherCount} phiếu · ${usage.userCount} tài khoản</small></td><td><button class="btn ghost" onclick="editWarehouseMaster(decodeURIComponent('${encodeURIComponent(row.name)}'))">Sửa</button> <button class="btn danger" onclick="deleteWarehouseMaster(decodeURIComponent('${encodeURIComponent(row.name)}'))" ${protectedWarehouse?'disabled title="Kho Chính và Kho Văn Phòng gắn với vai trò phân quyền"':''}>Xóa</button></td></tr>`;
    }).join('')||'<tr><td colspan="7">Chưa có kho</td></tr>';
  }
  const supplierTable=$('supplierMasterTable');
  if(supplierTable){
    const rows=(data.suppliers||[]).filter(row=>row.active!=='deleted').sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'vi'));
    supplierTable.innerHTML=rows.map(row=>{const usage=(data.stockVouchers||[]).filter(v=>v.supplierId===row.id||(!v.supplierId&&v.supplierName===row.name)).length;return `<tr><td><b>${htmlesc(row.code||'')}</b></td><td><b>${htmlesc(row.name||'')}</b></td><td>${htmlesc(row.contact||'')}</td><td>${htmlesc(row.phone||'')}</td><td>${htmlesc(row.taxCode||'')}</td><td>${htmlesc(row.address||'')}</td><td>${row.active==='inactive'?'<span class="badge orange">Tạm ẩn</span>':'<span class="badge green">Đang dùng</span>'}</td><td>${usage}</td><td><button class="btn ghost" onclick="editSupplierMaster('${row.id}')">Sửa</button> <button class="btn danger" onclick="deleteSupplierMaster('${row.id}')">Xóa</button></td></tr>`}).join('')||'<tr><td colspan="9">Chưa có nhà cung cấp. Hãy tạo nhà cung cấp trước khi lập phiếu nhập kho.</td></tr>';
  }
}
window.renderWarehouseMaster=renderWarehouseMaster;
window.resetWarehouseMasterForm=()=>{editingWarehouseMaster=null;['warehouseMasterId','warehouseMasterCode','warehouseMasterName','warehouseMasterAddress','warehouseMasterPhone','warehouseMasterNote'].forEach(id=>{if($(id))$(id).value=''});if($('warehouseMasterActive'))$('warehouseMasterActive').value='active';};
window.editWarehouseMaster=name=>{const row=warehouseMasterByName(name);if(!row)return alert('Không tìm thấy kho');editingWarehouseMaster={id:row.id||'',name:row.name,isDefault:!!row.isDefault,defaultName:warehouseOverrideDefaultName(row)};if($('warehouseMasterId'))$('warehouseMasterId').value=row.id||'';if($('warehouseMasterCode'))$('warehouseMasterCode').value=row.code||'';if($('warehouseMasterName'))$('warehouseMasterName').value=row.name||'';if($('warehouseMasterAddress'))$('warehouseMasterAddress').value=row.address||'';if($('warehouseMasterPhone'))$('warehouseMasterPhone').value=row.phone||'';if($('warehouseMasterActive'))$('warehouseMasterActive').value=row.active||'active';if($('warehouseMasterNote'))$('warehouseMasterNote').value=row.note||'';document.getElementById('warehouseMaster')?.scrollIntoView({behavior:'smooth',block:'start'});};
window.saveWarehouseMaster=async()=>{
  if(currentPerm.role!=='Admin')return alert('Chỉ Admin được thêm hoặc sửa danh mục kho.');
  const code=String($('warehouseMasterCode')?.value||'').trim().toUpperCase();
  const name=String($('warehouseMasterName')?.value||'').trim();
  const address=String($('warehouseMasterAddress')?.value||'').trim();
  const phone=String($('warehouseMasterPhone')?.value||'').trim();
  const active=$('warehouseMasterActive')?.value||'active';
  const note=String($('warehouseMasterNote')?.value||'').trim();
  if(!code||!name)return alert('Nhập mã kho và tên kho');
  const oldName=editingWarehouseMaster?.name||'';
  const duplicateName=warehouseMasterRows(true).some(row=>searchKey(row.name)===searchKey(name)&&searchKey(row.name)!==searchKey(oldName));
  const duplicateCode=warehouseMasterRows(true).some(row=>searchKey(row.code)===searchKey(code)&&searchKey(row.name)!==searchKey(oldName));
  if(duplicateName||duplicateCode)return alert('Mã kho hoặc tên kho đã tồn tại');
  if(oldName&&PROTECTED_WAREHOUSE_NAMES.includes(oldName)&&(name!==oldName||active!=='active'))return alert('Kho Chính và Kho Văn Phòng gắn với vai trò phân quyền nên không thể đổi tên hoặc tạm ẩn. Bạn vẫn có thể sửa mã, địa chỉ, điện thoại và ghi chú.');
  if(oldName&&active==='inactive'&&warehouseUsage(oldName).total>0)return alert('Kho đang có chứng từ, đơn bán hoặc tài khoản được phân quyền nên không thể tạm ẩn.');
  const payload={code,name,address,phone,active,note,updatedAt:serverTimestamp()};
  if(editingWarehouseMaster?.id)await updateDoc(doc(db,'warehouses',editingWarehouseMaster.id),payload);
  else if(oldName){await addDoc(col('warehouses'),{...payload,defaultName:oldName,createdAt:serverTimestamp()});}
  else await addDoc(col('warehouses'),{...payload,createdAt:serverTimestamp()});
  if(oldName&&oldName!==name)await renameWarehouseInRelatedData(oldName,name);
  await logAction(oldName?'Sửa kho':'Thêm kho',`${oldName||name}${oldName&&oldName!==name?' → '+name:''}`);
  resetWarehouseMasterForm();await loadAll();
};
window.deleteWarehouseMaster=async name=>{
  if(currentPerm.role!=='Admin')return alert('Chỉ Admin được xóa danh mục kho.');
  if(PROTECTED_WAREHOUSE_NAMES.includes(name))return alert('Kho Chính và Kho Văn Phòng được bảo vệ vì đang gắn với vai trò phân quyền.');
  const usage=warehouseUsage(name);if(usage.total>0)return alert(`Kho đang được sử dụng: ${usage.voucherCount} chứng từ, ${usage.saleCount} đơn bán, ${usage.userCount} tài khoản. Hãy chuyển dữ liệu/phân quyền trước khi xóa.`);
  const row=(data.warehouses||[]).find(item=>searchKey(item.name)===searchKey(name)&&item.active!=='deleted');
  if(!confirm('Xóa kho '+name+'?'))return;
  if(row?.id&&warehouseOverrideDefaultName(row))await updateDoc(doc(db,'warehouses',row.id),{active:'deleted',updatedAt:serverTimestamp()});
  else if(row?.id)await deleteDoc(doc(db,'warehouses',row.id));
  else await addDoc(col('warehouses'),{code:warehouseDefaultRecord(name)?.code||'',name,defaultName:name,active:'deleted',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
  await logAction('Xóa kho',name);await loadAll();
};
window.resetSupplierMasterForm=()=>{editingSupplier=null;['supplierMasterId','supplierMasterCode','supplierMasterName','supplierMasterContact','supplierMasterPhone','supplierMasterTaxCode','supplierMasterAddress','supplierMasterNote'].forEach(id=>{if($(id))$(id).value=''});if($('supplierMasterActive'))$('supplierMasterActive').value='active';};
window.editSupplierMaster=id=>{const row=supplierById(id);if(!row)return alert('Không tìm thấy nhà cung cấp');editingSupplier=id;['Code','Name','Contact','Phone','TaxCode','Address','Note'].forEach(key=>{const el=$('supplierMaster'+key);if(el)el.value=row[key.charAt(0).toLowerCase()+key.slice(1)]||''});if($('supplierMasterId'))$('supplierMasterId').value=id;if($('supplierMasterActive'))$('supplierMasterActive').value=row.active||'active';document.getElementById('supplierMasterForm')?.scrollIntoView({behavior:'smooth',block:'center'});};
window.saveSupplierMaster=async()=>{
  const payload={code:String($('supplierMasterCode')?.value||'').trim().toUpperCase(),name:String($('supplierMasterName')?.value||'').trim(),contact:String($('supplierMasterContact')?.value||'').trim(),phone:String($('supplierMasterPhone')?.value||'').trim(),taxCode:String($('supplierMasterTaxCode')?.value||'').trim(),address:String($('supplierMasterAddress')?.value||'').trim(),active:$('supplierMasterActive')?.value||'active',note:String($('supplierMasterNote')?.value||'').trim(),updatedAt:serverTimestamp()};
  if(!payload.code||!payload.name)return alert('Nhập mã và tên nhà cung cấp');
  if((data.suppliers||[]).some(row=>row.id!==editingSupplier&&(searchKey(row.code)===searchKey(payload.code)||searchKey(row.name)===searchKey(payload.name))))return alert('Mã hoặc tên nhà cung cấp đã tồn tại');
  if(editingSupplier)await updateDoc(doc(db,'suppliers',editingSupplier),payload);else await addDoc(col('suppliers'),{...payload,createdAt:serverTimestamp()});
  await logAction(editingSupplier?'Sửa nhà cung cấp':'Thêm nhà cung cấp',`${payload.code} - ${payload.name}`);resetSupplierMasterForm();await loadAll();
};
window.deleteSupplierMaster=async id=>{const row=supplierById(id);if(!row)return;const usage=(data.stockVouchers||[]).filter(v=>v.supplierId===id||(!v.supplierId&&v.supplierName===row.name)).length;if(usage>0)return alert(`Nhà cung cấp đang được dùng trong ${usage} phiếu nhập kho, không thể xóa. Có thể chuyển sang Tạm ẩn.`);if(!confirm('Xóa nhà cung cấp '+row.name+'?'))return;await deleteDoc(doc(db,'suppliers',id));await logAction('Xóa nhà cung cấp',`${row.code||''} - ${row.name}`);await loadAll();};


const STOCK_OPERATION_META={
  IN:{title:'Phiếu nhập kho',hint:'Nhập hàng theo nhà cung cấp vào đúng kho được phân quyền.'},
  OUT:{title:'Phiếu xuất kho',hint:'Xuất hàng ra khỏi hệ thống cho khách hàng, kỹ thuật, nhà cung cấp hoặc đối tượng khác.'},
  TRANSFER:{title:'Phiếu chuyển kho',hint:'Kho gửi xuất hàng; kho nhận kiểm đếm và xác nhận trước khi được cộng tồn.'},
...
function updateStockWorkflowHint(){
  ...
  if(type==='IN'&&stockInIsInternal()){
    ...
    const negativeHint=internalSource==='Kho Chính'&&source==='Kho Văn Phòng'?' Nếu Kho Chính bằng 0 hoặc thiếu tồn, hệ thống vẫn cho lưu sau khi xác nhận cảnh báo và sẽ ghi nhận tồn Kho Chính bị âm.':'';
    hint.className='stock-workflow-hint is-transfer';
    hint.innerHTML=`<b>Nhập nội bộ hai bước:</b> ${htmlesc(internalSource)} → ${htmlesc(source)}. Hệ thống lập phiếu chuyển kho; hàng chỉ cộng vào kho nhận sau khi xác nhận số lượng thực tế.${negativeHint}`;
  }else if(type==='OUT'){
    ...
    hint.className='stock-workflow-hint is-transfer';
    hint.innerHTML=`<b>Chuyển kho hai bước:</b> ${source} → ${destination}. Khi lưu, hàng chuyển sang <b>Đang vận chuyển</b>; chỉ cộng kho nhận sau khi kho nhận xác nhận số lượng thực tế.`;
  }else{
...
window.saveStockVoucher=async()=>{
  ...
  const officeReceiptFromMain=formType==='IN'&&internalSource==='Kho Chính'&&toWarehouse==='Kho Văn Phòng';
  ...
  for(const it of raw){
    ...
    if(it.cost<0)return alert('Giá vốn không được âm: '+it.code);
    const available=stockOf(it.code,editingId,warehouse);
    if((type==='OUT'||type==='TRANSFER')&&it.inputQty>available){
      if(officeReceiptFromMain)negativeSourceRows.push({code:it.code,available,qty:it.inputQty,after:available-it.inputQty});
      else return alert(`Không đủ tồn kho cho ${it.code} tại ${warehouse}. Tồn hiện có: ${available}`);
    }
    if(type==='CHECK'){
  ...
  if(negativeSourceRows.length){
    const detail=negativeSourceRows.map(row=>`${row.code}: tồn ${row.available}, nhập ${row.qty}, tồn Kho Chính sau chuyển ${row.after}`).join('\n');
    if(!confirm(`Kho Chính không đủ tồn nhưng Kho Văn Phòng vẫn được phép nhập.\n\n${detail}\n\nTiếp tục lưu và chấp nhận tồn Kho Chính bị âm?`))return;
  }
  ...
  let o={code:voucherCode,date:$('stockDate').value,type,warehouse:warehouse,fromWarehouse:warehouse,toWarehouse:type==='TRANSFER'?toWarehouse:'',entryMode:internalSource?'INTERNAL_RECEIPT':'',negativeSourceStock:negativeSourceRows.length>0,negativeSourceDetails:negativeSourceRows,supplierId:supplier?.id||'',supplierCode:supplier?.code||'',supplierName:supplier?.name||'',recipientType,recipientName,recipientPhone,recipientAddress,note:$('stockNote').value,items,value:items.reduce((a,it)=>a+Math.abs(+it.qty||0)*(+it.cost||0),0),updatedAt:serverTimestamp()};
  ...
  if(editingStock){if(!has('editStock'))return alert('Không có quyền sửa kho');const oldV=data.stockVouchers.find(x=>x.id===editingStock);if(oldV?.type==='TRANSFER')return alert('Phiếu chuyển kho không sửa trực tiếp sau khi gửi. Admin có thể hủy phiếu đang vận chuyển và lập phiếu mới.');if(stockVoucherLocked(oldV)&&!isCurrentAdmin())return alert('Phiếu kho đã liên kết đơn bán/đã khóa. Chỉ Admin được sửa.');await updateDoc(doc(db,'stockVouchers',editingStock),o);await logAction('Sửa phiếu kho',`${o.code} - ${stockTypeName(type)}`)}
    ...
    await logAction('Tạo phiếu kho',`${o.code} - ${stockTypeName(type)}${internalSource?` - Nhập nội bộ ${warehouse} → ${toWarehouse}`:(type==='TRANSFER'?' - Đang vận chuyển':'')}${negativeSourceRows.length?' - Cho phép tồn Kho Chính âm: '+negativeSourceRows.map(row=>`${row.code} ${row.after}`).join(', '):''}`)
  }
...
window.editStock=id=>{
  ...
  if($('stockSupplier')){$('stockSupplier').innerHTML=supplierOptions(v.supplierId||'');$('stockSupplier').value=v.supplierId||'';}
  if($('stockRecipientType'))$('stockRecipientType').value=v.recipientType||(v.type==='OUT'?'Khác':'Khách hàng');
  if($('stockRecipientName'))$('stockRecipientName').value=v.recipientName||v.toWarehouse||v.customerName||'';
  if($('stockRecipientPhone'))$('stockRecipientPhone').value=v.recipientPhone||v.customerPhone||'';
  if($('stockRecipientAddress'))$('stockRecipientAddress').value=v.recipientAddress||v.customerAddress||'';
...
window.openTransferReceipt=id=>{
  const v=data.stockVouchers.find(x=>x.id===id);
  if(!v||v.type!=='TRANSFER')return alert('Không tìm thấy phiếu chuyển kho');
  if(!canConfirmTransferVoucher(v))return alert('Chỉ kho nhận được xác nhận phiếu đang vận chuyển gửi đến kho mình.');
  document.getElementById('transferReceiptModal')?.remove();
  const rows=(v.items||[]).map((it,index)=>{const sent=Math.abs(+it.qty||0);return `<tr><td>${index+1}</td><td><b>${htmlesc(it.code||'')}</b></td><td>${htmlesc(it.name||'')}</td><td>${sent}</td><td><input class="transfer-received-qty" data-index="${index}" type="number" min="0" max="${sent}" step="1" value="${sent}"></td></tr>`}).join('');
  document.body.insertAdjacentHTML('beforeend',`<div class="modal-backdrop" id="transferReceiptModal"><div class="modal-card"><div class="panel-head"><div><h3>Xác nhận nhận hàng ${htmlesc(v.code||'')}</h3><span>${htmlesc(v.fromWarehouse||v.warehouse||'')} → ${htmlesc(v.toWarehouse||'')}</span></div><button class="btn ghost" onclick="document.getElementById('transferReceiptModal').remove()">Đóng</button></div><div class="alert"><b>Kiểm đếm thực tế trước khi xác nhận.</b> Số lượng nhận sẽ được cộng vào kho nhận. Hàng thiếu được ghi nhận chênh lệch và không tự hoàn về kho gửi.</div><table><thead><tr><th>STT</th><th>Model</th><th>Sản phẩm</th><th>SL gửi</th><th>SL thực nhận</th></tr></thead><tbody>${rows}</tbody></table><div style="margin-top:14px"><label>Ghi chú nhận hàng</label><textarea id="transferReceiveNote" rows="3" placeholder="Tình trạng kiện hàng, model thiếu/hỏng, biên bản kèm theo..."></textarea></div><div style="display:flex;justify-content:flex-end;gap:8px;margin-top:14px"><button class="btn ghost" onclick="document.getElementById('transferReceiptModal').remove()">Đóng</button><button class="btn primary" onclick="confirmTransferReceipt('${id}')">Xác nhận nhập kho</button></div></div></div>`);
};
window.confirmTransferReceipt=async id=>{
  const v=data.stockVouchers.find(x=>x.id===id);
  if(!v||!canConfirmTransferVoucher(v))return alert('Phiếu không còn ở trạng thái có thể xác nhận hoặc bạn không thuộc kho nhận.');
  const inputs=[...document.querySelectorAll('#transferReceiptModal .transfer-received-qty')];
  if(inputs.length!==(v.items||[]).length)return alert('Danh sách nhận hàng không hợp lệ. Vui lòng đóng và mở lại phiếu.');
  const receivedItems=[];
  for(const input of inputs){
    const index=+input.dataset.index;
    const it=(v.items||[])[index]||{};
    const sentQty=Math.abs(+it.qty||0);
    const receivedQty=Number(input.value);
    if(!Number.isFinite(receivedQty)||receivedQty<0||receivedQty>sentQty)return alert(`Số lượng thực nhận của ${it.code||'dòng '+(index+1)} phải từ 0 đến ${sentQty}.`);
    receivedItems.push({lineIndex:index,code:it.code||'',name:it.name||'',sentQty,receivedQty,shortageQty:Math.max(0,sentQty-receivedQty)});
  }
  const receiveNote=String($('transferReceiveNote')?.value||'').trim();
  const hasDifference=receivedItems.some(row=>row.shortageQty>0);
  if(hasDifference&&!receiveNote)return alert('Có chênh lệch nhận hàng. Bắt buộc nhập ghi chú để đối soát.');
  if(!confirm(hasDifference?'Xác nhận nhập số lượng thực nhận và ghi nhận phần thiếu để Admin đối soát?':'Xác nhận đã nhận đủ và cộng hàng vào kho nhận?'))return;
  const transferStatus=hasDifference?'RECEIVED_WITH_DIFFERENCE':'RECEIVED';
  await updateDoc(doc(db,'stockVouchers',id),{transferStatus,receivedItems,receivedAt:serverTimestamp(),receivedBy:normEmail(currentUser?.email||''),receiveNote,updatedAt:serverTimestamp()});
  await logAction('Xác nhận nhận hàng chuyển kho',`${v.code} | ${v.toWarehouse||''} | ${transferStatusLabel({...v,transferStatus})}${hasDifference?' | Thiếu '+receivedItems.reduce((sum,row)=>sum+row.shortageQty,0):''}`);
  document.getElementById('transferReceiptModal')?.remove();
  await loadAll();
};
window.cancelTransferVoucher=async id=>{
  ...
  if(!String(reason||'').trim())return alert('Bắt buộc nhập lý do hủy.');
  if(!confirm(`Xác nhận hủy ${v.code}? Tồn kho nguồn sẽ được hoàn lại và phiếu vẫn được giữ để truy vết.`))return;
...
window.printStock=id=>{
  const v=data.stockVouchers.find(x=>x.id===id);
  if(!v)return alert('Không tìm thấy chứng từ kho');
  if(!canAccessVoucher(v))return alert('Bạn không có quyền xem hoặc in chứng từ của kho này.');
  const wh=v.type==='TRANSFER'?`${v.fromWarehouse||v.warehouse||''} → ${v.toWarehouse||''}`:voucherWarehouse(v);
  const partner=v.type==='IN'?(v.supplierName||'Chưa có nhà cung cấp'):(v.type==='OUT'?(v.recipientName||v.toWarehouse||v.customerName||''):(v.type==='TRANSFER'?(v.toWarehouse||''):''));
  const partnerLabel=v.type==='IN'?'Nhà cung cấp':(v.type==='OUT'?`${v.recipientType||'Nơi nhận'}`:'Kho nhận');
  const partnerLine=partner?`<div><b>${partnerLabel}:</b> ${htmlesc(partner)}${v.type==='OUT'&&v.recipientPhone?` · ${htmlesc(v.recipientPhone)}`:''}</div>`:'<div></div>';
  const transferLine=v.type==='TRANSFER'?`<div style="grid-column:1/-1"><b>Trạng thái:</b> ${transferStatusLabel(v)}${v.receivedBy?` · Người nhận: ${htmlesc(v.receivedBy)}`:''}${v.receiveNote?` · Ghi chú nhận: ${htmlesc(v.receiveNote)}`:''}</div>`:'';
  const html=`<div class="print-a5">${printHeader(stockTypeName(v.type).toUpperCase())}
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px 18px;border-bottom:1px solid #999;padding-bottom:8px;margin-bottom:8px;line-height:1.55">
      ...
      <div><b>Kho:</b> ${wh}</div>${partnerLine}${transferLine}
      <div style="grid-column:1/-1"><b>Ghi chú:</b> ${v.note||''}</div>
    </div>
    <table><thead><tr><th>STT</th><th>Model</th><th>Tên sản phẩm</th><th>${v.type==='TRANSFER'?'SL gửi':'SL'}</th>${v.type==='TRANSFER'?'<th>SL nhận</th>':''}<th class="view-cost">Giá vốn</th><th>Ghi chú</th></tr></thead><tbody>${(v.items||[]).map((it,i)=>`<tr><td>${i+1}</td><td>${it.code||''}</td><td>${it.name||''}</td><td>${it.actualQty??it.inputQty??it.qty??0}</td>${v.type==='TRANSFER'?`<td>${transferIsReceived(v)?transferReceivedQty(v,it,i):'Chưa nhận'}</td>`:''}<td>${money(it.cost||0)}</td><td>${it.note||''}</td></tr>`).join('')}</tbody></table>
...
function renderStockBook(){
  ...
  const visibleWarehouses=allowed.filter(name=>activeWh==='ALL'||activeWh===name);
  const head=$('stockBookHead')||document.querySelector('#stockbook thead tr');
  if(head)head.innerHTML=`<th>Model</th><th>Sản phẩm</th><th>Nhập</th><th>Xuất</th><th>Chuyển kho</th><th>Đang vận chuyển</th><th>Chênh lệch nhận</th><th>Điều chỉnh</th>${visibleWarehouses.map(name=>`<th>${htmlesc(name)}</th>`).join('')}<th>Tổng tồn</th><th>Trạng thái</th><th class="view-cost">Giá vốn</th><th class="view-cost">Giá trị tồn</th>`;
  let rows=stockBookRows(dateFilter.from,dateFilter.to).map(r=>{
    let visibleStock=activeWh==='ALL'?r.stock:(r.warehouseStock[activeWh]||0);
    const visibleInTransit=activeWh==='ALL'?r.inTransit:(r.warehouseInTransit?.[activeWh]||0);
    const visibleDifference=activeWh==='ALL'?r.pendingDifference:(r.warehouseDifference?.[activeWh]||0);
    return {...r,visibleStock,visibleInTransit,visibleDifference,visibleValue:visibleStock*r.cost};
  }).filter(r=>{
  ...
  const sumInTransit=rows.reduce((a,r)=>a+r.visibleInTransit,0);
  const sumDifference=rows.reduce((a,r)=>a+r.visibleDifference,0);
  ...
  if($('stockBookSummary'))$('stockBookSummary').innerHTML=`
    ...
    ${visibleWarehouses.map(name=>`<div class="stock-summary-card"><span>${htmlesc(name)}</span><b>${rows.reduce((sum,row)=>sum+(row.warehouseStock[name]||0),0)}</b></div>`).join('')}
    <div class="stock-summary-card"><span>Tổng tồn đang xem</span><b>${sumStock}</b></div>
    <div class="stock-summary-card"><span>Đang vận chuyển</span><b>${sumInTransit}</b></div>
    <div class="stock-summary-card"><span>Chênh lệch chờ xử lý</span><b>${sumDifference}</b></div>
    ${has('viewCost')?`<div class="stock-summary-card"><span>Giá trị tồn</span><b>${money(sumValue)}</b></div>`:''}`;
  if($('stockBookTable'))$('stockBookTable').innerHTML=rows.map(r=>`<tr><td><b>${r.code}</b></td><td>${r.name}</td><td>${r.totalIn}</td><td>${r.totalOut}</td><td>${r.totalTransfer}</td><td><b>${r.visibleInTransit}</b></td><td>${r.visibleDifference?`<span class="badge red">${r.visibleDifference}</span>`:'0'}</td><td>${r.totalAdj}</td>${visibleWarehouses.map(name=>`<td><b>${r.warehouseStock[name]||0}</b></td>`).join('')}<td><b>${r.visibleStock}</b></td><td>${stockStatusBadge(r.visibleStock,r.minStock)}</td><td class="view-cost">${money(r.cost)}</td><td class="view-cost"><b>${money(r.visibleValue)}</b></td></tr>`).join('')||`<tr><td colspan="${12+visibleWarehouses.length}">Không tìm thấy tồn kho phù hợp</td></tr>`;
  applyPermissions();
}
...
function reportDateValue(v){
  ...
  else if(typeof v==='object'&&Number.isFinite(+v.seconds))d=new Date(+v.seconds*1000);
  else if(raw)d=new Date(raw);
  return d&&!Number.isNaN(d.getTime())?isoDateFromLocal(d):'';
}
...
function stockReportVoucherRows(from,to,typeFilter=''){
  ...
  activeStockVouchers().filter(canAccessVoucher).forEach(v=>{
    ...
    (v.items||[]).forEach((it,index)=>{
      const qty=+(it.actualQty??it.inputQty??it.qty??0)||0;
      rows.push({
        ...
        supplierName:v.supplierName||'', supplierCode:v.supplierCode||'', customer:v.customerName||'', saleCode:v.saleCode||'',
        recipientType:v.recipientType||(v.type==='OUT'?'Khác':''),recipientName:v.recipientName||v.toWarehouse||v.customerName||'',recipientPhone:v.recipientPhone||v.customerPhone||'',
        transferStatus:transferStatusOf(v),transferStatusName:transferStatusLabel(v),receivedQty:v.type==='TRANSFER'?transferReceivedQty(v,it,index):qty,
        product:it.code||'', name:it.name||'',qty, cost:+(it.cost||0)||0,
        amount:qty*(+(it.cost||0)||0), note:it.note||v.note||''
      });
    });
  });
...
function renderStockReports(from,to){
  ...
  const rows=stockBookRows('', '').map(r=>{
    const visibleStock=wh==='ALL'?r.stock:(r.warehouseStock[wh]||0);
    const visibleInTransit=wh==='ALL'?r.inTransit:(r.warehouseInTransit?.[wh]||0);
    const visibleDifference=wh==='ALL'?r.pendingDifference:(r.warehouseDifference?.[wh]||0);
    return {...r,visibleStock,visibleInTransit,visibleDifference,visibleValue:visibleStock*r.cost};
  }).filter(r=>{
  ...
  const totalInTransit=rows.reduce((a,r)=>a+r.visibleInTransit,0);
  const totalDifference=rows.reduce((a,r)=>a+r.visibleDifference,0);
  ...
  $('reportStockSummary').innerHTML=`
    ...
    <div class="report-card">Đang vận chuyển<b>${totalInTransit}</b></div>
    <div class="report-card">Chênh lệch nhận<b>${totalDifference}</b></div>
  ...
  const visibleWarehouses=allowed.filter(name=>wh==='ALL'||wh===name);
  if($('reportStockHead'))$('reportStockHead').innerHTML=`<tr><th>Model</th><th>Sản phẩm</th>${visibleWarehouses.map(name=>`<th>${htmlesc(name)}</th>`).join('')}<th>Đang vận chuyển</th><th>Chênh lệch nhận</th><th>Tổng tồn</th><th>Trạng thái</th><th class="view-cost">Giá vốn</th><th class="view-cost">Giá trị tồn</th></tr>`;
  if($('reportStockTable'))$('reportStockTable').innerHTML=rows.map(r=>`<tr><td><b>${r.code}</b></td><td>${r.name||''}</td>${visibleWarehouses.map(name=>`<td>${r.warehouseStock[name]||0}</td>`).join('')}<td>${r.visibleInTransit}</td><td>${r.visibleDifference}</td><td><b>${r.visibleStock}</b></td><td>${stockStatusBadge(r.visibleStock,r.minStock)}</td><td class="view-cost">${money(r.cost)}</td><td class="view-cost"><b>${money(r.visibleValue)}</b></td></tr>`).join('')||`<tr><td colspan="${8+visibleWarehouses.length}">Không tìm thấy tồn kho phù hợp</td></tr>`;
  const renderVoucher=(arr,kind)=>arr.slice(0,400).map(r=> kind==='out'
    ? `<tr><td>${r.date}</td><td>${r.code}</td><td>${r.warehouse}</td><td>${r.recipientType||''}</td><td>${r.recipientName||r.saleCode||''}</td><td><b>${r.product}</b></td><td>${r.name}</td><td>${r.qty}</td><td class="view-cost">${money(r.amount)}</td><td>${r.note}</td></tr>`
    : `<tr><td>${r.date}</td><td>${r.code}</td><td>${r.warehouse}</td><td>${r.supplierName||''}</td><td><b>${r.product}</b></td><td>${r.name}</td><td>${r.qty}</td><td class="view-cost">${money(r.amount)}</td><td>${r.note}</td></tr>`).join('');
  if($('reportStockInTable'))$('reportStockInTable').innerHTML=renderVoucher(inRows,'in')||'<tr><td colspan="9">Chưa có nhập kho trong kỳ</td></tr>';
  if($('reportStockOutTable'))$('reportStockOutTable').innerHTML=renderVoucher(outRows,'out')||'<tr><td colspan="10">Chưa có xuất kho trong kỳ</td></tr>';
  if($('reportStockMoveTable'))$('reportStockMoveTable').innerHTML=moveRows.slice(0,400).map(r=>`<tr><td>${r.date}</td><td>${r.code}</td><td>${r.typeName}</td><td>${r.fromWarehouse||''}</td><td>${r.toWarehouse||r.warehouse||''}</td><td>${r.type==='TRANSFER'?(r.transferStatusName||'Đã nhận (phiếu cũ)'):'—'}</td><td><b>${r.product}</b></td><td>${r.qty}</td><td>${r.type==='TRANSFER'?r.receivedQty:r.qty}</td><td>${r.note}</td></tr>`).join('')||'<tr><td colspan="10">Chưa có điều chuyển / điều chỉnh trong kỳ</td></tr>';
...
function renderDebtReports(from,to){
  ...
  const all=calcDebtRows().filter(d=>{
    const sd=d.settled?(debtSettledDate(d)||debtSaleDate(d)||''):(debtSaleDate(d)||'');
...
function reportEmployeeIncomeRows(from,to){
  ...
  const staffId=canViewAllCommissions()?'':(currentStaffMatchIds()[0]||'__NO_STAFF__');
...
function renderReports(){
  ...
  const revenueMetrics=revenueDeductionMetrics(sales,expenses,salaries);
  const rev=revenueMetrics.revenue;
  ...
  $('reportBox').innerHTML=`
    ...
    <div class="report-card view-cost">Doanh thu sau chi phí + hoa hồng<b>${money(revenueMetrics.afterExpenseCommission)}</b><small>= Tổng doanh thu - chi phí - hoa hồng</small></div>
    <div class="report-card">Đơn ghi nhận / Sản phẩm<b>${sales.length} đơn / ${qty} SP</b></div>
    <div class="report-card">Lũy kế đã thu của đơn ghi nhận<b>${money(paid)}</b></div>
    <div class="report-card">Còn nợ<b>${money(debt)}</b></div>
    <div class="report-card">Khoản thu chưa làm phiếu đủ 100%<b>${money(pendingDepositCollected)}</b><small>${pendingDepositSales.length} đơn chưa được tính doanh thu</small></div>
  ...
  if($('reportProfitBreakdownTable'))$('reportProfitBreakdownTable').innerHTML=[
    ...
    ['Doanh thu sau chi phí', revenueMetrics.afterExpense, 'Tổng doanh thu - tổng chi phí chưa gồm hoa hồng'],
    ['Tổng hoa hồng Sale', -revenueMetrics.commission, 'Chỉ hoa hồng của các phiếu đã thu đủ 100% trong kỳ'],
    ['Doanh thu sau hoa hồng', revenueMetrics.afterCommission, 'Tổng doanh thu - hoa hồng Sale'],
    ['Doanh thu sau chi phí + hoa hồng', revenueMetrics.afterExpenseCommission, 'Tổng doanh thu - tổng chi phí chưa gồm hoa hồng - hoa hồng Sale'],
  ...
  sales.forEach(s=>(s.items||[]).forEach(it=>{
    ...
    const line=saleItemCommissionBaseShare(s,it);
  ...
  if($('reportRevenueDetailTable'))$('reportRevenueDetailTable').innerHTML=sales.slice().sort((a,b)=>String(saleRevenueRecognitionDate(b)).localeCompare(String(saleRevenueRecognitionDate(a)))).map(s=>{const ci=saleCustomerInfo(s),sum=saleItemSummary(s);return `<tr><td><b>${saleRevenueRecognitionDate(s)}</b></td><td>${s.date||''}</td><td><b>${htmlesc(s.code||'')}</b></td><td>${htmlesc(ci.name||'')}</td><td>${htmlesc(ci.source||'Chưa cập nhật')}</td><td>${htmlesc(sum.models||'')}</td><td class="text-center">${sum.totalQty||0}</td><td>${money(s.grand)}</td><td>${htmlesc(s.staffName||'')}</td><td>${htmlesc(s.techName||'')}</td></tr>`}).join('')||'<tr><td colspan="10">Chưa có doanh thu đủ điều kiện ghi nhận trong kỳ</td></tr>';

  const byTime={};
  sales.forEach(s=>{
    const k=groupKeyByPeriod(saleRevenueRecognitionDate(s),period);
    byTime[k]=byTime[k]||{key:k,orders:0,qty:0,revenue:0,surcharge:0,paid:0,debt:0,comm:0,tech:0,profit:0};
    const pay=salePaymentInfo(s);byTime[k].orders++;byTime[k].qty+=(s.items||[]).reduce((a,it)=>a+(+it.qty||0),0);byTime[k].revenue+=+s.grand||0;byTime[k].surcharge+=+s.surcharge||0;byTime[k].paid+=+pay.paidTotal||0;byTime[k].debt+=+pay.debtLeft||0;byTime[k].comm+=saleExpectedCommissionValue(s);byTime[k].tech+=(+s.techCost||0)+(+s.techFuel||0);byTime[k].profit+=saleProfitValue(s);
  });
  if($('reportRevenueTable'))$('reportRevenueTable').innerHTML=Object.values(byTime).sort((a,b)=>String(b.key).localeCompare(String(a.key))).map(x=>`<tr><td><b>${x.key}</b></td><td>${x.orders}</td><td>${x.qty}</td><td>${money(x.revenue)}</td><td>${money(x.surcharge)}</td><td>${money(x.paid)}</td><td>${money(x.debt)}</td><td class="view-cost">${money(x.comm)}</td><td class="view-cost">${money(x.tech)}</td><td class="view-cost">${money(x.profit)}</td></tr>`).join('')||'<tr><td colspan="10">Chưa có doanh thu đủ điều kiện trong kỳ</td></tr>';
  const timeAsc=Object.values(byTime).sort((a,b)=>String(a.key).localeCompare(String(b.key)));
  renderChartHtml('reportRevenueCharts',
    modernLineChart('Xu hướng doanh thu ghi nhận',timeAsc.map(x=>({label:x.key,value:x.revenue})),{sub:'Theo ngày phiếu đạt đủ 100%',money:true,badge:money(rev)})+
    modernBarChart('Top model bán chạy',productRows.map(x=>({label:x.code,value:x.qty})),{sub:'Số lượng bán ra theo model',limit:8})+
    modernBarChart('Doanh thu theo model',productRows.map(x=>({label:x.code,value:x.revenue})),{sub:'Top model tạo doanh thu đã ghi nhận',money:true,limit:8})
  );
  renderChartHtml('reportProfitCharts',
    modernBarChart('Cơ cấu lợi nhuận',[{label:'Doanh thu trước VAT',value:revenueBeforeVat},{label:'Giá vốn',value:totalCost},{label:'Lãi gộp',value:grossMargin},{label:'HH dự kiến',value:projectedComm},{label:'Công + xăng KT',value:tech},{label:'Chi phí CTY',value:totalCompanyCost},{label:'Lợi nhuận ròng',value:profit}],{sub:'Các chỉ tiêu của doanh thu đã ghi nhận',money:true,limit:8})+
    ...
    modernDonutChart('Chi phí vận hành',[{label:'Chi phí khác',value:op},{label:'Lương nhân viên',value:sal},{label:'HH dự kiến',value:projectedComm},{label:'Kỹ thuật + xăng',value:tech}],{sub:'Các khoản chi phí chính',money:true})
  );
  ...
  returnVouchers.forEach(v=>(v.items||[]).forEach(it=>{
    ...
    returnRows.push({date:v.date,code:v.code,saleCode:v.saleCode||sale.code||'',customer:v.customerName||saleCustomerInfo(sale).name||'',warehouse:voucherWarehouse(v),product:it.code,name:it.name||'',qty:+it.qty||0,amount:returnVoucherItemAmount(v,it),settlement:v.settlement||sale.returnSettlement||'',note:v.note||''});
  }));
...
function renderPermissions(){
  ...
  if($('permissionTable'))$('permissionTable').innerHTML=rows.map(u=>{const effective=normalizePermission(u).warehouseAccess;return `<tr><td><b>${u.email||''}</b><br><small>UID: ${u.id}</small></td><td>${u.name||''}</td><td><span class="badge green">${u.role||''}</span></td><td><div class="perm-chip-wrap">${permissionSummary(u.perms||[])}</div></td><td>${effective.map(w=>`<span class="perm-chip">${w}</span>`).join('')||'<span class="muted-small">Không giới hạn/Chưa chọn</span>'}</td><td><button class="btn ghost" onclick="editPermission('${u.id}')">Sửa</button> <button class="btn ghost admin-only" onclick="adminSendPasswordReset('${u.id}')">Reset mật khẩu</button></td></tr>`}).join('')||'<tr><td colspan="6">Không tìm thấy phân quyền phù hợp</td></tr>';
  applyPermissions();
}
...
window.saveUserPermission=async()=>{
  ...
  if(role==='Kho'&&!warehouseAccess.length)warehouseAccess=WAREHOUSES;
  if(role==='Kho Chính'||role==='Kho Văn Phòng')warehouseAccess=[role];
...
window.cancelSale=async(id)=>{
  ...
  const commissionHistory=Array.isArray(s.commissionHistory)?s.commissionHistory.slice():[];
  const recognizedCommission=saleCommissionValue(s);
  if(recognizedCommission>0)commissionHistory.push({type:'canceled',date:today(),amount:-recognizedCommission,reason:reason.trim(),at:new Date().toISOString()});
  await updateDoc(doc(db,'sales',id),{canceled:true,isCanceled:true,status:'Đã hủy',paymentStatus:'Đã hủy',orderStatus:'Đã hủy',commissionStatus:'canceled',commissionEarnedAt:'',commissionHistory,cancelReason:reason.trim(),cancelledAt:serverTimestamp(),cancelledBy:currentUser?.email||'',paidTotal:0,debtLeft:0,updatedAt:serverTimestamp()});
  await cancelRelatedSaleDocs(s,reason.trim());
  await logAction('Hủy phiếu bán',`${s.code} | ${reason.trim()} | Doanh thu -${money(s.grand)} | HH -${money(recognizedCommission)}`);
  await loadAll();
...
window.removeDoc=async(name,id)=>{
  ...
  if(name==='receipts'){
    ...
    if(currentPerm.role!=='Admin')return alert('Chỉ Admin được hủy phiếu thu đã lưu.');
    if(!r)return alert('Không tìm thấy phiếu thu');
    const reason=prompt('Nhập lý do hủy phiếu thu (bắt buộc):','');
    if(!String(reason||'').trim())return alert('Bắt buộc nhập lý do hủy phiếu thu.');
    const saleId=receiptSaleId(r);
    customerToRefresh=r.customerId||'';
    await updateDoc(doc(db,'receipts',id),{canceled:true,isCanceled:true,status:'Đã hủy',cancelReason:String(reason).trim(),cancelledAt:serverTimestamp(),cancelledBy:currentUser?.email||'',updatedAt:serverTimestamp()});
    await logAction('Hủy phiếu thu',`${r.code||id} | ${r.saleCode||''} | ${money(r.amount||0)} | ${reason}`);
    await loadAll();
    if(saleId)await updatePaymentStatusForSaleSnapshot(saleId);
    else if(customerToRefresh)await updatePaymentStatusesForCustomer(customerToRefresh);
    await loadAll();
    return;
  }
...
const excelSchemas={
  warehouses:{sheet:'Danh_muc_kho',headers:['code','name','address','phone','active','note'],sample:[]},
  suppliers:{sheet:'Nha_cung_cap',headers:['code','name','contact','phone','taxCode','address','active','note'],sample:[]},
  ...
  stockVouchers:{sheet:'Chung_tu_kho',headers:['code','date','type','warehouse','toWarehouse','supplierCode','supplierName','recipientType','recipientName','recipientPhone','recipientAddress','transferStatus','productCode','productName','qty','receivedQty','cost','note'],sample:[{code:'NK000001',date:today(),type:'IN',warehouse:defaultWarehouse(),toWarehouse:'',supplierCode:'NCC001',supplierName:'Nhà cung cấp mẫu',recipientType:'',recipientName:'',recipientPhone:'',recipientAddress:'',transferStatus:'',productCode:'F07',productName:'Khóa thông minh F07',qty:10,receivedQty:'',cost:950000,note:'Nhập kho'}]},
  ...
  returns:{sheet:'Hang_tra_lai',headers:['date','voucherCode','saleCode','customer','warehouse','code','name','qty','amount','settlement','paymentMethod','note'],sample:[]},
...
function exportRows(type){let rows=[];
  if(type==='warehouses')rows=warehouseMasterRows(true).map(row=>({code:row.code||'',name:row.name||'',address:row.address||'',phone:row.phone||'',active:row.active||'active',note:row.note||''}));
  if(type==='suppliers')rows=(data.suppliers||[]).filter(row=>row.active!=='deleted').map(row=>({code:row.code||'',name:row.name||'',contact:row.contact||'',phone:row.phone||'',taxCode:row.taxCode||'',address:row.address||'',active:row.active||'active',note:row.note||''}));
  if(type==='customers')rows=data.customers.map(c=>({customerCode:ensureCustomerCode(c),name:c.name,type:c.type,phone:c.phone,address:c.address,email:c.email||'',contact:c.contact||'',source:c.source||'',birthday:c.birthday||'',note:c.note||'',discount:c.discount,openingDebt:c.openingDebt}));
  if(type==='products')rows=data.products.map(p=>({code:p.code,name:p.name,category:p.category,cost:p.cost,price:p.price,minStock:p.minStock,active:p.active||'active',stock:visibleStockOf(p.code)}));
  ...
  if(type==='stockVouchers')rows=activeStockVouchers().filter(canAccessVoucher).flatMap(v=>(v.items||[]).map((it,index)=>({code:v.code,date:v.date,type:v.type,warehouse:voucherWarehouse(v),fromWarehouse:v.fromWarehouse||v.warehouse||'',toWarehouse:v.type==='TRANSFER'?(v.toWarehouse||''):'',supplierCode:v.supplierCode||'',supplierName:v.supplierName||'',recipientType:v.recipientType||(v.type==='OUT'?'Khác':''),recipientName:v.recipientName||v.toWarehouse||v.customerName||'',recipientPhone:v.recipientPhone||v.customerPhone||'',recipientAddress:v.recipientAddress||v.customerAddress||'',transferStatus:v.type==='TRANSFER'?transferStatusOf(v):'',productCode:it.code,productName:it.name,qty:it.actualQty??it.inputQty??it.qty,receivedQty:v.type==='TRANSFER'?(transferIsReceived(v)?transferReceivedQty(v,it,index):0):'',cost:it.cost,note:it.note||v.note||''})));
  ...
  if(type==='returns'){rows=activeStockVouchers().filter(v=>v.type==='RETURN'&&canAccessVoucher(v)).flatMap(v=>(v.items||[]).map(it=>{const sale=data.sales.find(s=>s.id===v.saleId||s.code===v.saleCode)||{};return{date:v.date||'',voucherCode:v.code||'',saleCode:v.saleCode||sale.code||'',customer:v.customerName||saleCustomerInfo(sale).name||'',warehouse:voucherWarehouse(v),code:it.code||'',name:it.name||'',qty:+it.qty||0,amount:returnVoucherItemAmount(v,it),settlement:v.settlement||sale.returnSettlement||'',paymentMethod:v.paymentMethod||'',note:v.note||''}}));}
  if(type==='cashbook'){let balance=+$('cashbookOpening')?.value||0;rows=cashbookRows().map(r=>{balance+=(+r.income||0)-(+r.expense||0);return{date:r.date,code:r.code,type:r.type,content:r.content,paymentMethod:r.paymentMethod,income:r.income,expense:r.expense,balance}});}
  ...
  if(type==='stockbook'){const df=stockBookDateFilter();rows=stockBookRows(df.from,df.to).filter(r=>!df.active||r.periodMovement).map(r=>({tuNgay:df.from||'',denNgay:df.to||'',model:r.code,sanPham:r.name,nhap:r.totalIn,xuat:r.totalOut,chuyenKho:r.totalTransfer,dangVanChuyen:r.inTransit,chenhLechNhan:r.pendingDifference,dieuChinh:r.totalAdj,...Object.fromEntries(userWarehouses().map(name=>[name,r.warehouseStock[name]||0])),tongTonHienTai:r.stock,giaVon:has('viewCost')?r.cost:'Ẩn',giaTriTon:has('viewCost')?r.value:'Ẩn'}));}
  return rows;
}
...
function revenueSummaryExportRows(){
  const range=reportRange();
  const sales=revenueRecognizedSalesInRange(range.from,range.to);
  const expenses=data.expenses.filter(e=>inReportRange(e.date,range.from,range.to)&&!isSalaryCategory(e.category));
  const salaries=data.salaries.filter(e=>inReportRange(e.date,range.from,range.to));
  const m=revenueDeductionMetrics(sales,expenses,salaries);
  return [{
    'Từ ngày':range.from,'Đến ngày':range.to,'Số phiếu đã thu đủ 100%':sales.length,
    'Tổng doanh thu':m.revenue,'Tổng chi phí chưa gồm hoa hồng':m.expense,
    'Tổng hoa hồng Sale':m.commission,'Doanh thu sau chi phí':m.afterExpense,
    'Doanh thu sau hoa hồng':m.afterCommission,
    'Doanh thu sau chi phí + hoa hồng':m.afterExpenseCommission,
    'Công thức chi phí':'Giá vốn + công/xăng kỹ thuật + phiếu chi + lương'
  }];
}
function profitLossExportData(){
  const range=reportRange();
  const sales=revenueRecognizedSalesInRange(range.from,range.to);
  const expenses=data.expenses.filter(e=>inReportRange(e.date,range.from,range.to)&&!isSalaryCategory(e.category));
  const salaries=data.salaries.filter(e=>inReportRange(e.date,range.from,range.to));
  const revenueGross=sales.reduce((a,s)=>a+(+s.grand||0),0);
  const vat=sales.reduce((a,s)=>a+(+s.vat||0),0);
  const revenueNet=sales.reduce((a,s)=>a+saleCommissionBaseValue(s),0);
  const productCost=sales.reduce((a,s)=>a+saleCostValue(s),0);
  const commission=sales.reduce((a,s)=>a+saleCommissionValue(s),0);
  const techCost=sales.reduce((a,s)=>a+(+s.techCost||0),0);
  const techFuel=sales.reduce((a,s)=>a+(+s.techFuel||0),0);
  const operatingExpense=expenses.reduce((a,e)=>a+(+e.amount||0),0);
  const salaryExpense=salaries.reduce((a,e)=>a+(+e.total||+e.amount||0),0);
  const grossProfit=revenueNet-productCost;
  const orderProfit=grossProfit-commission-techCost-techFuel;
  const netProfit=orderProfit-operatingExpense-salaryExpense;
  return {range,sales,expenses,salaries,revenueGross,vat,revenueNet,productCost,grossProfit,commission,techCost,techFuel,orderProfit,operatingExpense,salaryExpense,netProfit};
}
function profitLossSummaryExportRows(){
  const p=profitLossExportData();
  const result=p.netProfit>=0?'LÃI':'LỖ';
  return [
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Doanh thu đã thu đủ (gồm VAT)','Số tiền':p.revenueGross,'Công thức / Ghi chú':`${p.sales.length} phiếu đạt đủ 100% trong kỳ`},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'VAT','Số tiền':-p.vat,'Công thức / Ghi chú':'Tách khỏi doanh thu để tính lãi lỗ'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Doanh thu trước VAT','Số tiền':p.revenueNet,'Công thức / Ghi chú':'Doanh thu gồm VAT - VAT'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Giá vốn sản phẩm','Số tiền':-p.productCost,'Công thức / Ghi chú':'Giá vốn của các đơn được ghi nhận'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Lãi gộp','Số tiền':p.grossProfit,'Công thức / Ghi chú':'Doanh thu trước VAT - Giá vốn'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Hoa hồng Sale','Số tiền':-p.commission,'Công thức / Ghi chú':'Tính ngay khi phiếu đạt đủ 100%'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Công kỹ thuật','Số tiền':-p.techCost,'Công thức / Ghi chú':'Theo từng phiếu bán'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Tiền xăng kỹ thuật','Số tiền':-p.techFuel,'Công thức / Ghi chú':'Theo từng phiếu bán'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Lợi nhuận đơn hàng','Số tiền':p.orderProfit,'Công thức / Ghi chú':'Lãi gộp - Hoa hồng - Công - Xăng'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Chi phí vận hành','Số tiền':-p.operatingExpense,'Công thức / Ghi chú':'Phiếu chi trong kỳ, không gồm lương'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':'Lương nhân viên','Số tiền':-p.salaryExpense,'Công thức / Ghi chú':'Bảng lương trong kỳ'},
    {'Từ ngày':p.range.from,'Đến ngày':p.range.to,'Chỉ tiêu':`KẾT QUẢ ${result}`,'Số tiền':p.netProfit,'Công thức / Ghi chú':'Lợi nhuận đơn hàng - Chi phí vận hành - Lương'}
  ];
}
function profitLossDetailExportRows(){
  const p=profitLossExportData();
  return p.sales.slice().sort((a,b)=>String(saleRevenueRecognitionDate(a)).localeCompare(String(saleRevenueRecognitionDate(b)))).map(s=>{
    const ci=saleCustomerInfo(s),sum=saleItemSummary(s);
    const revenueNet=saleCommissionBaseValue(s),cost=saleCostValue(s),commission=saleCommissionValue(s),techCost=+s.techCost||0,techFuel=+s.techFuel||0;
    return {'Ngày ghi nhận':saleRevenueRecognitionDate(s),'Ngày bán':s.date||'','Mã phiếu':s.code||'','Khách hàng':ci.name||'','Nguồn khách hàng':ci.source||'','Model':sum.models||'','Số lượng':sum.totalQty||0,'Doanh thu gồm VAT':+s.grand||0,'VAT':+s.vat||0,'Doanh thu trước VAT':revenueNet,'Giá vốn':cost,'Lãi gộp':revenueNet-cost,'Hoa hồng Sale':commission,'Công kỹ thuật':techCost,'Tiền xăng KT':techFuel,'Lợi nhuận đơn hàng':revenueNet-cost-commission-techCost-techFuel};
  });
}
function profitLossExpenseExportRows(){
  const p=profitLossExportData();
  return p.expenses.map(e=>({'Ngày':reportDateValue(e.date),'Loại chi phí':e.category||'','Số tiền':+e.amount||0,'Phương thức':paymentMethodText(e.paymentMethod),'Ghi chú':e.note||''}));
}
function profitLossSalaryExportRows(){
  const p=profitLossExportData();
  return p.salaries.map(e=>({'Ngày':reportDateValue(e.date),'Nhân viên':e.staffName||'','Lương cơ bản':+e.base||0,'Phụ cấp':+e.allowance||0,'Thưởng':+e.bonus||0,'Phạt':+e.deduct||0,'Tổng lương':+(e.total||e.amount)||0,'Ghi chú':e.note||''}));
}
function profitLossWorkbookSheets(){
  const sheets={'Tong_hop_lai_lo':profitLossSummaryExportRows(),'Chi_tiet_don_hang':profitLossDetailExportRows(),'Chi_phi_van_hanh':profitLossExpenseExportRows()};
  // Không để lộ lương từng nhân viên cho tài khoản chỉ có quyền xem lợi nhuận.
  if(has('viewSalary')||has('manageSalary'))sheets['Luong_nhan_vien']=profitLossSalaryExportRows();
  return sheets;
}
...
function commissionExportRowsForSales(rows){
  return rows.slice().sort((a,b)=>String(saleCommissionRecognitionDate(a)).localeCompare(String(saleCommissionRecognitionDate(b)))||String(saleCommissionEarnedAt(a)).localeCompare(String(saleCommissionEarnedAt(b)))).map((s,idx)=>{const parts=commissionPartsForFilter(s),pay=salePaymentInfo(s),ci=saleCustomerInfo(s);return {
    ...
    'Khách hàng':ci.name||'',
    'SĐT':ci.phone||'',
    'Nguồn khách hàng':ci.source||'',
    'Model':saleItemSummary(s).models,
    'Số lượng':saleItemSummary(s).totalQty,
    'Chi tiết SL':saleItemSummary(s).qtyText,
    'Sale':commissionStaffMeta(s,'Sale').name||'',
    'Kỹ thuật':commissionStaffMeta(s,'Kỹ thuật').name||'',
    'Tiền hàng':+(s.subtotal||0)||0,
    'Tổng đã thu':pay.paidTotal,
    'Còn nợ':pay.debtLeft,
    'Phụ thu':parts.includeSale?(+(s.surcharge||0)||0):0,
    'Chiết khấu':parts.includeSale?(+(s.discountTotal||0)||0):0,
    'Doanh số tính HH':parts.includeSale?saleCommissionBaseValue(s):0,
    '% HH':parts.includeSale?(+(s.commissionPercent||0)||0):0,
    'Hoa hồng Sale':parts.saleCommission,
    'Công kỹ thuật':parts.techCost,
    'Tiền xăng KT':parts.techFuel,
    'Tổng nhận':parts.saleCommission+parts.techCost+parts.techFuel
  }});
}
...
window.exportExcel=(type)=>{try{
  if(type==='profitLossReport'&&!has('viewCost'))return alert('Tài khoản chưa có quyền xem giá vốn/lợi nhuận.');
  if(['commissions','techPerformance','employeeIncome'].includes(type)){
    const ownStaff=canViewAllCommissions()?'':(currentStaffMatchIds()[0]||'__NO_STAFF__');
    if($('reports')?.classList.contains('active')){const range=reportRange();commissionAppliedFilter={q:reportSearchQ(),dept:type==='techPerformance'?'Kỹ thuật':'',staffId:ownStaff,from:range.from,to:range.to};}
    else commissionAppliedFilter=readCommissionFilterFromForm();
  }
  const schema=excelSchemas[type]||{sheet:type};
  const rows=exportRows(type);
  const sheets=type==='profitLossReport'
    ? profitLossWorkbookSheets()
    : type==='revenueReport'
  ...
  const range=type==='profitLossReport'?reportRange():null;
  const filename=type==='profitLossReport'?`Bao_cao_doanh_thu_lai_lo_${range.from}_den_${range.to}.xlsx`:`${type}_${today()}.xlsx`;
...
window.exportAllExcel=()=>{try{const oldFilter=commissionAppliedFilter;commissionAppliedFilter={q:'',dept:'',staffId:canViewAllCommissions()?'':(currentStaffMatchIds()[0]||'__NO_STAFF__'),from:'',to:''};const sheets={};['warehouses','suppliers','customers','products','prices','staff','sales','stockVouchers','expenses','salaries','warranties','commissions','stockbook','returns','logs'].forEach(t=>{sheets[excelSchemas[t]?.sheet||t]=exportRows(t)});commissionAppliedFilter=oldFilter;const wb=makeWorkbook(sheets);XLSX.writeFile(wb,`Similock_Da_Nang_Toan_bo_${today()}.xlsx`);}catch(err){alert(err.message)}};
...
function normalizeImportDate(v,fallback=''){
...
window.importExcel=async(e,type)=>{
  ...
  const existingSalaryKey=new Map(data.salaries.map(x=>[`${reportDateValue(x.date)}|${x.staffId||searchKey(x.staffName||'')}`,x]));
  ...
  for(let r=0;r<rows.length;r++){
    ...
    try{
      if(type==='customers'){
        ...
        if(obj.discount<0||obj.discount>100||obj.openingDebt<0){skip++;errors.push(`Dòng ${r+2}: chiết khấu/công nợ đầu kỳ không hợp lệ`);continue}
        ...
        if(obj.cost<0||obj.price<0||obj.minStock<0){skip++;errors.push(`Dòng ${r+2}: giá/tồn tối thiểu không được âm`);continue}
        ...
        if(!obj.code||obj.price<=0){skip++;errors.push(`Dòng ${r+2}: thiếu model hoặc giá phải lớn hơn 0`);continue}
        ...
        const commissionMissing=obj.commissionPercent===undefined||obj.commissionPercent===null||String(obj.commissionPercent).trim()==='';
        const techFeeMissing=obj.techFee===undefined||obj.techFee===null||String(obj.techFee).trim()==='';
        ...
        if(obj.functions.includes('Sale')||obj.functions.includes('Quản lý')){if(commissionMissing)obj.commissionPercent=5}else obj.commissionPercent=0;
        if(obj.functions.includes('Kỹ thuật')){if(techFeeMissing)obj.techFee=100000}else obj.techFee=0;
        ...
        const date=normalizeImportDate(obj.date||obj['Ngày']||obj['Ngay']||'');
        const staffName=String(obj.staffName||obj['Nhân viên']||obj['Nhan vien']||'').trim();
        const staff=data.staff.find(x=>x.id===obj.staffId)||existingStaff.get(staffName.toLowerCase())||staffByIdentity('',staffName)||{};
        const base=safeNum(obj.base??obj['Lương cơ bản']??obj['Luong co ban']);
        const allowance=safeNum(obj.allowance??obj['Phụ cấp']??obj['Phu cap']);
        const bonus=safeNum(obj.bonus??obj['Thưởng']??obj['Thuong']);
        const deduct=safeNum(obj.deduct??obj['Khấu trừ']??obj['Khau tru']);
        if(!date||!staff.id){skip++;errors.push(`Dòng ${r+2}: ngày không hợp lệ hoặc nhân viên chưa có trong danh mục`);continue;}
        if([base,allowance,bonus,deduct].some(x=>x<0)){skip++;errors.push(`Dòng ${r+2}: các khoản lương không được âm`);continue;}
        const salary={date,staffId:staff.id,staffName:staff.name||staffName,base,allowance,bonus,deduct,total:base+allowance+bonus-deduct,note:String(obj.note||obj['Ghi chú']||obj['Ghi chu']||''),updatedAt:serverTimestamp()};
        const key=`${date}|${staff.id}`;
        if(existingSalaryKey.has(key))await updateDoc(doc(db,'salaries',existingSalaryKey.get(key).id),salary);else await addDoc(col('salaries'),{...salary,createdAt:serverTimestamp()});
      }else if(type==='expenses'){
        obj.date=String(obj.date||today());obj.category=obj.category||'Khác';obj.amount=safeNum(obj.amount);obj.note=obj.note||'';
        if(!validateDate(obj.date)||obj.amount<=0){skip++;errors.push(`Dòng ${r+2}: ngày không hợp lệ hoặc số tiền phải lớn hơn 0`);continue}
        ...
        if(!o.code||o.cost<=0){skip++;continue;}
        ...
        if(!canAccessWarehouse(importWarehouse)){skip++;errors.push(`Dòng ${r+2}: không có quyền nhập chứng từ cho ${importWarehouse}`);continue;}
        const recipientType=String(obj.recipientType||obj['Loại nơi nhận']||'').trim();
        const recipientName=String(obj.recipientName||obj['Đối tượng/Nơi nhận']||'').trim();
        const recipientPhone=String(obj.recipientPhone||'').trim();
        const recipientAddress=String(obj.recipientAddress||'').trim();
        if(voucherType==='TRANSFER'&&!transferDestinationWarehouses(importWarehouse).includes(toWarehouse)){skip++;errors.push(`Dòng ${r+2}: kho nhận không hợp lệ hoặc trùng kho chuyển đi`);continue;}
        if(voucherType==='OUT'&&(!recipientType||!recipientName)){skip++;errors.push(`Dòng ${r+2}: phiếu OUT thiếu loại nơi nhận hoặc đối tượng/nơi nhận`);continue;}
        if(voucherType==='IN'&&!supplier){skip++;errors.push(`Dòng ${r+2}: nhà cung cấp chưa có trong danh mục`);continue;}
        const key=code+'|'+voucherType+'|'+(obj.date||today())+'|'+importWarehouse+'|'+toWarehouse+'|'+(supplier?.id||'')+'|'+recipientType+'|'+recipientName;
        ...
        if(!item.code||item.qty<=0||item.cost<0){skip++;errors.push(`Dòng ${r+2}: thiếu model, số lượng phải lớn hơn 0 hoặc giá vốn bị âm`);continue}
        if(!stockGroup.has(key)) stockGroup.set(key,{code,date:String(obj.date||today()),type:voucherType,warehouse:importWarehouse,fromWarehouse:importWarehouse,toWarehouse:voucherType==='TRANSFER'?toWarehouse:'',supplierId:supplier?.id||'',supplierCode:supplier?.code||'',supplierName:supplier?.name||'',recipientType:voucherType==='OUT'?recipientType:'',recipientName:voucherType==='OUT'?recipientName:'',recipientPhone:voucherType==='OUT'?recipientPhone:'',recipientAddress:voucherType==='OUT'?recipientAddress:'',...(voucherType==='TRANSFER'?{transferStatus:'IN_TRANSIT',receivedItems:[],dispatchedBy:currentUser?.email||''}:{}),note:obj.note||'',items:[]});
        ...
        if(items.some(it=>(+it.qty||0)<=0||(+it.price||0)<0)){skip++;errors.push(`Dòng ${r+2}: số lượng/đơn giá không hợp lệ`);continue}
        const code=String(obj.code||nextCode('BH',data.sales)).trim();
        const saleDate=String(obj.date||today()).slice(0,10);
        const paid=safeNum(obj.paid);
        const surcharge=safeNum(obj.surcharge);
        if(!validateDate(saleDate)||paid<0||surcharge<0){skip++;errors.push(`Dòng ${r+2}: ngày hoặc số tiền không hợp lệ`);continue}
        const totals=calcSaleTotals(items,obj.vatMode||'included8',paid,surcharge,obj.orderDiscountType||'none',safeNum(obj.orderDiscountValue));
        const importedGrand=safeNum(obj.grand);
        if(importedGrand&&Math.abs(importedGrand-totals.grand)>1){skip++;errors.push(`Dòng ${r+2}: tổng tiền ${importedGrand} không khớp công thức ${Math.round(totals.grand)}`);continue}
        const grand=totals.grand;
        if(paid>grand){skip++;errors.push(`Dòng ${r+2}: đã thu lớn hơn tổng tiền`);continue}
        const staff=existingStaff.get(String(obj.staffName||'').trim().toLowerCase())||data.staff.find(x=>x.id===obj.staffId)||{};
        const tech=existingStaff.get(String(obj.techName||'').trim().toLowerCase())||data.staff.find(x=>x.id===obj.techId)||{};
        const customer=existingByPhone.get(normalizePhone(obj.customerPhone))||data.customers.find(x=>debtClean(ensureCustomerCode(x))===debtClean(obj.customerCode||''))||{};
        const commissionMissing=obj.commissionPercent===undefined||obj.commissionPercent===null||String(obj.commissionPercent).trim()==='';
        const techCostMissing=obj.techCost===undefined||obj.techCost===null||String(obj.techCost).trim()==='';
        const commissionPercent=commissionMissing?+(staff.commissionPercent??5):safeNum(obj.commissionPercent);
        const techCost=techCostMissing&&tech.id?+(tech.techFee??100000):safeNum(obj.techCost);
        const techFuel=safeNum(obj.techFuel);
        if(commissionPercent<0||commissionPercent>100||techCost<0||techFuel<0){skip++;errors.push(`Dòng ${r+2}: hoa hồng/công kỹ thuật không hợp lệ`);continue}
        const commissionBase=calcCommissionBase(totals);
        const expectedSaleCommission=Math.round(commissionBase*commissionPercent/100);
        const cost=items.reduce((a,it)=>a+costFor(it.code,saleDate)*(+it.qty||0),0);
        const fullyPaid=grand>0&&paid>=grand;
        const directPaidDate=paid>0?String(obj.directPaidDate||obj.commissionEarnedAt||saleDate).slice(0,10):'';
        const earnedDate=fullyPaid?String(obj.commissionEarnedAt||directPaidDate||saleDate).slice(0,10):'';
        const installStatus=String(obj.installStatus||'Chưa lắp').trim();
        const installCompletedDate=installStatus==='Đã lắp'?String(obj.installCompletedDate||saleDate).slice(0,10):'';
        const customerSource=String(obj.customerSource||customer.source||'').trim();
        const customerCode=obj.customerCode||ensureCustomerCode(customer)||customerCodeFromPhone(obj.customerPhone);
        const customerName=obj.customerName||customer.name||'';
        const customerPhone=obj.customerPhone||customer.phone||'';
        const customerAddress=obj.customerAddress||customer.address||'';
        const customerType=obj.customerType||customer.type||'Khách lẻ';
        const o={code,date:saleDate,customerId:customer.id||'',customerCode,customerName,customerPhone,customerAddress,customerType,customerGroup:customerType,customerSource,customerSnapshot:{id:customer.id||'',code:customerCode,name:customerName,phone:customerPhone,address:customerAddress,type:customerType,source:customerSource},staffId:staff.id||'',staffName:staff.name||obj.staffName||'',techId:tech.id||'',techName:tech.name||obj.techName||'',items,...totals,grand,paid,debt:Math.max(0,grand-paid),paidTotal:paid,debtLeft:Math.max(0,grand-paid),paidSource:paid>0?'excel_import':'',paidEntryKey:paid>0?code:'',directPaidDate,directPaidLocked:paid>0,paymentMethod:obj.paymentMethod||'Tiền mặt',commissionPercent,commissionBase,expectedSaleCommission,saleCommission:fullyPaid?expectedSaleCommission:0,commissionEarnedAt:earnedDate,commissionStatus:fullyPaid?'earned':'pending',commissionHistory:fullyPaid?[{type:'earned',date:earnedDate,amount:expectedSaleCommission,reason:'Import Excel đã thu đủ',at:new Date().toISOString()}]:[],techCost,techFuel,surcharge,cost,profit:commissionBase-cost-expectedSaleCommission-techCost-techFuel,status:fullyPaid?'Đã thu tiền':(paid>0?'Thanh toán một phần':'Chưa thu tiền'),paymentStatus:fullyPaid?'Đã thu tiền':(paid>0?'Thanh toán một phần':'Chưa thu tiền'),installStatus,installCompletedDate,note:obj.note||'',createdAt:serverTimestamp()};
...
function resetLocalDataAfterClear(){
  ['customers','products','staff','prices','costPrices','sales','warehouses','suppliers','stockVouchers','receipts','warranties','warrantyReasons','systemCategories','expenses','salaries','logs'].forEach(n=>{data[n]=[]});
}
window.clearAllData=async()=>{
  ...
  try{
    ...
    const collectionsToClear=['customers','products','prices','costPrices','staff','sales','warehouses','suppliers','stockVouchers','receipts','warranties','warrantyReasons','systemCategories','expenses','salaries','logs'];
    let result=[];
...
Object.assign(window,{
  ...
  renderWarehouseMaster, renderReports, renderPermissions, renderAuditLogs
});
